<template>
    <div class="boardHeader">
        <div style="display:flex;min-height:40px;width: 100%;">
            <div style="width:70%;display:flex;min-height:40px;width: calc(100% - 163px);">
                <div @click="$emit('closeMe')" style="width: 40px;
                    margin-left:5px;
                    height: 40px;
                    min-width: 40px;
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    border-radius:50px;"
                    @touchstart="$event.currentTarget.classList.add('mobileHeaderButton')">      
                                                    
                    <svg class="dot-menu" version="1.1" width="15" height="15" viewBox="0 0 20 32" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0.775 17.789c1.305 1.166 2.612 2.332 3.927 3.486 1.311 1.161 2.634 2.308 3.953 3.46 1.316 1.156 2.646 2.296 3.973 3.439 1.33 1.139 2.667 2.273 4.015 3.394 0.662 0.551 1.647 0.52 2.272-0.107 0.65-0.654 0.619-1.725-0.020-2.393-1.198-1.253-2.407-2.495-3.621-3.729-1.232-1.245-2.462-2.492-3.704-3.725-0.902-0.9-1.803-1.802-2.707-2.699-0.033-0.032-0.055-0.069-0.072-0.106-0.045-0.036-0.082-0.080-0.111-0.129-0.069-0.047-0.129-0.117-0.176-0.216-0.021-0.047-0.044-0.092-0.066-0.136-0.12-0.062-0.214-0.168-0.246-0.325-0.001-0.005-0.002-0.009-0.003-0.014-0.104-0.157-0.187-0.327-0.254-0.505-0.109-0.185-0.182-0.388-0.226-0.601-0.002-0.012-0.005-0.024-0.007-0.036-0.016-0.085-0.028-0.172-0.036-0.259-0.195-0.593-0.26-1.183 0.030-1.653 0.006-0.157 0.067-0.277 0.157-0.361 0.019-0.050 0.039-0.099 0.063-0.149 0.040-0.084 0.1-0.145 0.17-0.188 0.008-0.015 0.019-0.028 0.028-0.042 0.032-0.13 0.106-0.228 0.202-0.293 0.072-0.145 0.157-0.287 0.26-0.43 0.046-0.063 0.101-0.113 0.163-0.151 0.018-0.020 0.037-0.038 0.059-0.054 0.014-0.059 0.044-0.116 0.094-0.165 0.9-0.888 1.797-1.782 2.699-2.672 1.244-1.231 2.476-2.475 3.714-3.717l1.843-1.871 1.832-1.885c0.655-0.681 0.669-1.793-0.044-2.48-0.652-0.631-1.693-0.624-2.385-0.038l-1.964 1.66-1.995 1.71c-1.32 1.149-2.648 2.293-3.962 3.45s-2.636 2.308-3.943 3.474c-1.311 1.159-3.284 2.806-4.106 3.689s-0.792 2.492 0.191 3.369z"></path>
                    </svg>                                        
                    
                </div>
                <div :style="{maxWidth: openedBoard.private_flag == 0 ? 'calc(100% - 120px)' : '100%'}">
                    <BoardTitle :item="openedBoard" titleStyle="font-weight:600;font-size:14px;line-height: 40px;" titleClass="board-title text"/>  
                </div>
                <div v-if="openedBoard && openedBoard.private_flag == 0" style="margin-left: 10px;white-space: nowrap;position:relative;height:40px">
                    <div @click.stop="viewBoardMembers" style="overflow:hidden;height:40px">
                        <div style="display: flex;height:40px;align-items:center">
                            <div :key="user.id" v-for="user in confirmedMembers.slice(0, 3)" style="position:relative;">                                
                                <UserIconPreLoad size="15" :user="user.user" imgClass="userSmallIcon"/>                                           
                            </div>                                                                                                                  
                            <p @touchstart.stop style="margin-top:2px;cursor:pointer;font-size:12px;margin-left: 3px;" v-if="confirmedMembers.length > 3">({{confirmedMembers.length}})</p>                                            
                        </div> 
                    </div>                    
                </div>

            </div>
            <div style="width: 163px;display:flex;min-height:40px;justify-content: flex-end;fill:var(--primary-color);">
                <div @click="$parent.$emit('startPrivateSearch')" style="width:40px;height:40px;display:flex;border-radius:50px" @touchstart="$event.currentTarget.classList.add('mobileHeaderButton')">
                    <svg style="margin:auto" version="1.1" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 32 32">
                        <path d="M31.875 28.185c-0.034-0.444-0.159-0.888-0.376-1.275-0.102-0.194-0.239-0.387-0.387-0.547-0.171-0.194-0.239-0.251-0.342-0.353-0.752-0.752-1.526-1.492-2.278-2.232-0.387-0.376-0.763-0.74-1.15-1.116l-0.865-0.831-0.091-0.091c-0.034-0.034-0.080-0.068-0.125-0.102-0.080-0.068-0.171-0.137-0.262-0.194-0.729-0.49-1.651-0.626-2.471-0.376-0.148 0.046-0.285 0.091-0.421 0.159-0.068 0.034-0.148 0.023-0.205-0.034-0.251-0.262-0.854-0.9-1.139-1.207-0.057-0.068-0.068-0.159-0.011-0.228 0.717-0.911 1.275-1.902 1.697-2.938 0.592-1.469 0.888-3.029 0.888-4.589s-0.296-3.12-0.888-4.601c-0.592-1.469-1.492-2.847-2.676-4.043-1.173-1.196-2.54-2.095-4.009-2.688-1.469-0.604-3.029-0.9-4.589-0.9-1.549 0-3.109 0.296-4.578 0.9-1.469 0.592-2.847 1.492-4.031 2.688-1.184 1.184-2.084 2.562-2.676 4.031s-0.888 3.041-0.888 4.601 0.296 3.12 0.888 4.589c0.592 1.469 1.492 2.847 2.676 4.043s2.562 2.084 4.031 2.688c1.469 0.604 3.029 0.9 4.589 0.9s3.12-0.296 4.578-0.9c1.036-0.421 2.038-1.002 2.949-1.72 0.046-0.034 0.114-0.034 0.159 0.011 0.273 0.273 1.002 0.957 1.253 1.196 0.034 0.034 0.046 0.091 0.023 0.137-0.205 0.444-0.307 0.945-0.285 1.446 0.023 0.421 0.137 0.854 0.342 1.23 0.102 0.194 0.228 0.376 0.364 0.535 0.171 0.194 0.228 0.251 0.33 0.353 0.74 0.774 1.469 1.549 2.209 2.3l1.116 1.15 0.558 0.569 0.376 0.376c0.034 0.034 0.080 0.080 0.125 0.114 0.080 0.068 0.171 0.137 0.262 0.205 0.74 0.512 1.708 0.683 2.574 0.444 0.433-0.114 0.843-0.319 1.196-0.615 0.046-0.034 0.091-0.068 0.125-0.114l0.114-0.102 0.421-0.421c0.319-0.319 0.558-0.706 0.717-1.127s0.216-0.877 0.182-1.321zM15.795 21.159c-1.15 0.467-2.391 0.706-3.621 0.706s-2.46-0.239-3.621-0.706c-1.15-0.467-2.243-1.173-3.177-2.118-0.945-0.945-1.64-2.027-2.118-3.189-0.467-1.162-0.706-2.403-0.706-3.633 0-1.241 0.239-2.471 0.706-3.633s1.173-2.243 2.118-3.189c0.945-0.957 2.027-1.651 3.189-2.13 1.15-0.467 2.38-0.706 3.621-0.706 1.23 0 2.46 0.239 3.621 0.706 1.15 0.467 2.232 1.173 3.177 2.118v0c0.945 0.945 1.64 2.027 2.118 3.189 0.467 1.162 0.706 2.403 0.706 3.633 0 1.241-0.239 2.471-0.706 3.633s-1.173 2.243-2.118 3.189c-0.957 0.957-2.038 1.663-3.189 2.13zM29.153 28.823l-0.478 0.478c-0.057 0.057-0.137 0.091-0.216 0.114-0.159 0.046-0.342 0.011-0.478-0.080-0.011-0.011-0.034-0.023-0.046-0.034l-0.068-0.068-0.285-0.273-1.708-1.674c-0.763-0.752-1.526-1.48-2.3-2.221-0.239-0.239-0.251-0.239-0.319-0.342-0.057-0.080-0.091-0.182-0.102-0.285-0.034-0.205 0.046-0.433 0.182-0.592 0.125-0.159 0.364-0.399 0.558-0.535 0.273-0.194 0.604-0.125 0.797 0.068s1.697 1.754 2.061 2.141c0.74 0.763 1.48 1.537 2.232 2.289 0.239 0.239 0.239 0.239 0.285 0.33 0.034 0.068 0.057 0.159 0.068 0.239 0.011 0.159-0.057 0.319-0.182 0.444z"></path>
                    </svg>
                </div>
                <div @click="goTask" style="width:40px;height:40px;display:flex;border-radius:50px;position: relative;" @touchstart="$event.currentTarget.classList.add('mobileHeaderButton')">
                    
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 37 32" style="margin:auto">
                        <path d="M36.297 0.493c-0.529-0.407-1.289-0.312-1.742 0.177l-2.463 2.656-2.479 2.698c-1.644 1.805-3.295 3.607-4.927 5.425-1.633 1.815-3.274 3.625-4.9 5.446-0.906 1.016-1.818 2.030-2.726 3.046-0.293 0.329-0.814 0.303-1.073-0.054-0.061-0.083-0.124-0.169-0.187-0.252l-0.538-0.737-1.64-2.19c-0.726-0.977-1.471-1.94-2.22-2.9l-1.134-1.428c-0.384-0.472-0.767-0.947-1.16-1.413-0.435-0.515-1.21-0.637-1.791-0.225-0.567 0.401-0.704 1.19-0.355 1.792 0.296 0.513 0.607 1.020 0.914 1.528l0.961 1.551c0.652 1.030 1.306 2.056 1.978 3.069l1.509 2.284 0.509 0.755c0.68 1.007 1.366 2.011 2.070 3.003l0.082 0.115c0.095 0.133 0.207 0.252 0.339 0.36 0.794 0.645 1.97 0.495 2.63-0.283 1.569-1.848 3.105-3.724 4.657-5.585 1.564-1.876 3.113-3.766 4.667-5.649 1.558-1.882 3.096-3.779 4.641-5.67l2.304-2.852 2.291-2.858c0.436-0.547 0.358-1.364-0.22-1.809z"></path>
                        <path d="M30.798 13.688c-0.736 0.045-1.297 0.682-1.307 1.417l-0.182 13.496c-0.004 0.298-0.247 0.532-0.545 0.527-1.719-0.029-3.439-0.041-5.158-0.055l-7.281-0.017-7.281-0.001-5.073 0.015c-0.257 0-0.465-0.21-0.462-0.466 0.019-1.7 0.019-3.398 0.019-5.098l-0.026-7.281-0.026-7.279-0.033-5.239c-0.001-0.21 0.168-0.38 0.378-0.381 1.558-0.010 3.114-0.023 4.671-0.031l20.184-0.204c0.809-0.008 1.46-0.691 1.409-1.517-0.046-0.754-0.701-1.326-1.457-1.334l-20.136-0.204c-2.244-0.012-4.486-0.037-6.729-0.038-0.915 0-1.66 0.739-1.667 1.655v0.010l-0.049 7.281-0.024 7.279-0.026 7.281c0 2.427 0 4.854 0.055 7.279l0.001 0.037c0.022 0.925 0.777 1.67 1.709 1.673l7.281 0.022 7.281-0.003 7.281-0.018c2.427-0.018 4.854-0.029 7.281-0.106l0.074-0.003c0.86-0.026 1.542-0.736 1.531-1.603l-0.212-15.725c-0.015-0.787-0.68-1.421-1.482-1.372z"></path>
                    </svg>
                    <p v-if="taskBadge" class="notification" style="top: 0px;right: 5px;z-index: 5;left: auto;top: 5px;height: 12px;min-width: 12px;font-size: 9px;">{{ taskBadge}}</p> 
                </div>
                <div @click="goFile" style="width:40px;height:40px;display:flex;border-radius:50px" @touchstart="$event.currentTarget.classList.add('mobileHeaderButton')">
                    <svg width="20" height="20" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 39 32" style="margin:auto">
                        <path d="M38.918 8.499c-0.078-1.516-0.946-2.591-2.41-2.734-3.602-0.35-8.939-0.376-12.813-0.324-0.402 0-0.751-0.259-0.881-0.635-0.324-0.933-0.933-2.669-1.257-3.368-0.453-0.985-1.153-1.347-2.474-1.425-1.594-0.078-9.6 0.013-9.6 0.013h-5.377c-0.363 0.026-0.726 0.13-1.036 0.311s-0.596 0.428-0.803 0.725c-0.104 0.155-0.194 0.311-0.259 0.479l-0.052 0.13-0.013 0.104-0.039 0.117-1.244 3.628c-0.091 0.272-0.233 0.674-0.311 1.036-0.091 0.376-0.155 0.751-0.194 1.14-0.013 0.091-0.013 0.194-0.026 0.285v0.259l-0.026 0.492c-0.026 0.648-0.052 1.296-0.065 1.943-0.026 1.296-0.039 2.591-0.039 3.874 0.013 1.296 0.026 2.578 0.065 3.874 0.065 2.578 0.402 6.517 0.544 7.747s0.181 2.306 0.415 3.226c0.22 0.907 0.79 1.892 1.917 2.034 2.332 0.311 8.045 0.531 10.364 0.557 2.63 0.026 7.929 0.026 11.077-0.052 3.524-0.104 8.486-0.376 11.543-0.583 1.451-0.104 2.073-0.738 2.202-1.892 0.751-6.866 0.998-16.829 0.79-20.962zM22.892 5.441c-0.013 0.013 0 0 0 0zM19.291 29.474c-3.77 0.026-11.427-0.168-15.145-0.35-0.311-0.013-0.868-0.415-0.92-0.894-0.013-0.13-0.026-0.272-0.039-0.376l-0.155-1.879c-0.091-1.257-0.155-2.526-0.207-3.796s-0.078-2.539-0.104-3.809c-0.039-2.539-0.013-5.079 0.052-7.618 0.026-0.635 0.039-1.27 0.078-1.892l0.026-0.479 0.013-0.22c0-0.065 0.013-0.13 0.013-0.194 0.026-0.259 0.078-0.505 0.13-0.764 0.065-0.259 0.13-0.466 0.246-0.803l0.933-2.721 0.168-0.505c0.078-0.22 0.285-0.363 0.518-0.363l13.759 0.013c0.259 0 0.492 0.168 0.583 0.415l0.57 1.529 0.674 1.866c0.13 0.324 0.337 0.622 0.583 0.868s0.544 0.441 0.868 0.57c0.168 0.065 0.648 0.181 1.386 0.181 3.161-0.013 10.196 0.091 12.528 0.207 0.194 0.013 0.324 0.168 0.337 0.35 0.298 3.446 0.104 15.326-0.181 19.718-0.026 0.35-0.298 0.635-0.661 0.635-4.664 0.104-10.934 0.285-16.052 0.311z"></path>
                    </svg> 
                </div>
                <div @click="goMemo" style="width:40px;height:40px;display:flex;border-radius:50px;margin-right:3px" @touchstart="$event.currentTarget.classList.add('mobileHeaderButton')">
                    
                    <svg style="margin:auto" height="19" width="19" class="appIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 27">
                        <path d="M29.997 22.391l-.079-15.74-.015-2.303-.004-.384v-.096a3.72 3.72 0 0 0-.894-2.447 3.74 3.74 0 0 0-2.94-1.312c-.466-.003-1.829.021-2.304.015C17.398.167 10.225-.015 3.752.001 1.776.036.074 1.756.052 3.732l-.004.798-.044 16.893c.004.239-.006.927-.001 1.15-.091 2.106 1.624 3.962 3.731 4.026l21.745-.027.577-.001a3.88 3.88 0 0 0 3.739-2.623c.179-.492.217-1.043.202-1.557m-2.595-.382l-.004.737a1.25 1.25 0 0 1-1.197 1.197l-19.917-.035-1.536.002-.768.001h-.094a1.2 1.2 0 0 1-.912-.427c-.223-.253-.302-.583-.285-.91l-.001-1.15L2.646 4.53l-.003-.737c.004-.381.194-.742.5-.975.205-.155.448-.244.702-.254l1.49-.008 18.427-.163v.045l1.535.01.769.005c.149-.001.307.008.448.054.524.144.944.646.98 1.188l-.092 18.314M6.188 8.827l8.834.193 8.835-.238c.548-.037.998-.473 1.037-1.036.041-.616-.423-1.148-1.037-1.188l-6.627-.224c-3.628-.036-7.422.012-11.042.179-1.458.128-1.458 2.19 0 2.314"/>
                        <path d="M23.948 12.156l-6.661-.223c-3.646-.036-7.46.012-11.099.178-1.458.129-1.458 2.192 0 2.314 2.91.136 5.966.188 8.88.193 2.925-.015 5.967-.047 8.879-.235.547-.037.999-.474 1.036-1.038.044-.615-.42-1.146-1.035-1.189m-.085 5.599l-6.628-.223c-3.629-.035-7.426.013-11.045.178-1.458.129-1.459 2.191 0 2.315 2.895.135 5.936.188 8.836.192 2.91-.016 5.938-.047 8.837-.236.547-.038.999-.474 1.036-1.037.042-.614-.421-1.146-1.036-1.189"/>
                    </svg>

                </div>               
            </div>
        </div>      
    </div> 
</template>

<script>
import UserIconPreLoad from '../Board/Mixed/UserIcon.vue'
    export default {
        props: ['openedBoard'],
        data(){
            return{
                
            }
        },
        components:{
            UserIconPreLoad
        },
        mounted() {
        },
        computed: {
            confirmedMembers(){
                const allMembers = this.openedBoard.board_to_users;
                return allMembers
            },
            taskBadge(){
                const nm = this.$store.state.taskBadge && this.openedBoard ? this.$store.state.taskBadge[this.openedBoard.id] : null
                return  nm > 99 ? '+99' : nm
            },
            boardMembers(){
                return this.openedBoard && this.openedBoard.board_to_users && this.openedBoard.board_to_users.length ? this.openedBoard.board_to_users : []
            }
        },
        methods:{
            userListMobile: function (item) {
                if (item.board_to_users !== undefined) {
                    var user_list = item.board_to_users.map(obj => obj.user).map(obj => obj.name);
                    if (user_list.length > 1) {
                        var sliced = user_list.slice(0, 1).join("　") + " ..." + "(" + user_list.length + ")";
                        return sliced;
                    }
                    return user_list.join("　");
                }
            },
            goFile(){
                // this.$emit('setAnimation', false)
                this.$store.commit('setmTransition', 'smLeave')
                this.$store.commit('setMenu', {name: '', id: null})
                setTimeout(() =>{
                    // this.$router.push({name: 'file',  query: {folder_id: 'root'}})
                    this.$router.push({ name: 'file'})
                },0) 
            },
            goTask(){
                // this.$store.commit('setmTransition', 'smLeave')
                this.$store.commit('setMenu', {name: '', id: null})
                setTimeout(() =>{
                    this.$router.push({name: 'task'})
                },0) 
            },
            goMemo(){
                this.$store.commit('setmTransition', 'smLeave')
                this.$store.commit('setMenu', {name: '', id: null})
                setTimeout(() =>{
                    this.$router.push({name: 'memo'})
                },0) 
            },
            badgeFilter(number){
                if(number == -1){
                    return '1';
                }
                else if(number > 99){
                    return '+99';
                }else{
                    return number;
                }
            },
            viewBoardMembers(){
                // this.$store.commit('setMenu', {name: 'boardMembersMb', id: 96})
                this.$parent.$emit('viewMembers', this.openedBoard)
            }
        }
    }
</script>
