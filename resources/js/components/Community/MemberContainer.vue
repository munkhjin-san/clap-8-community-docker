<template>
    <div ref="container" @scroll="handleScroll" class="w-full h-full bg-[var(--bg2)] overflow-x-hidden overflow-y-auto">
        <div class="mem-header-section" :style="{ 'transform': `translateY(${offset}px)` }">
            <div class="post-header">
                <div class="post-search-wrap member-search-wrap">
                    <PostSearchBar @search-start="(word) => { keyword = word }" className="newChatMemberSearch"
                        :customPlaceHolder="`コンタクト検索`" />
                </div>
            </div>
        </div>
                ...
                <div class="member-container" @scroll="scrollListen" @click="menu.close()" ref="memberContainerRef">
                    <div v-for="(position, index) in positions" style="">
                        ...
                        <div v-if="position.employees" class="employee-container">
                            <MemberBlock v-for="member in position.employees" :key="member.id" :member="member" />
                        </div>
                    </div>
                </div>
        ...
        <FloatButton :order="2" @action="() => router.push({name: 'today-comments'})" title="みんなのひとこと">
            <template #icon>
...
const getMembers = async () => {
    ...
    if(response.today_unread_comments && response.today_unread_comments.length) {
        router.push({name: 'today-comments'})
    }
    setTimeout(() => {
