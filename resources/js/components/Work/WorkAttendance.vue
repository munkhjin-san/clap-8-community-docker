        ...
        <div class="work-modal-inner" @mousedown.stop>
            <div class="recordFormTitle">
                ...
                <div @click="$emit('closeModal')" class="cursor-pointer" style="display:flex;align-items:center;margin: auto 0 auto auto;">
                    <button class="work-delete-button" @click.stop="attendanceCreate" v-if="!attendanceData.attendance_flag && (this.auth_user.id == 610 || this.auth_user.id == 608)">休業確定</button>
                    <button class="work-delete-button" @click.stop="deleteAttendance" v-if="attendanceData.attendance_flag && (this.auth_user.id == 610 || this.auth_user.id == 608)">勤怠確定を取り下げる</button>
            ...
            <LoaderButton style="margin-top:30px;" @triggered="attendanceConfirm" :loading="loading" :content="buttonTexts"/>
...
<script>
    ...
    export default{
        ...
        computed: {
            disableButton(){
                return this.attendanceData.unapproved_count > 0 || this.attendanceData.attendance_flag === true
            },
            buttonTexts(){
                if(this.attendanceData.unapproved_count > 0){
                    return '未承認日報があります'
                }else if(this.attendanceData.attendance_flag){
                    return '確定済み'
                }else{
                    return '今月の勤怠を確定する'
                }
        ...
        methods: {
            deleteAttendance(){
                ...
                emitter.on(uniqueChannell, (data) => { 
                    if(data.answer === this.$t('confirmToAction')){
                        axios.post('attendance_delete', params).then(
                            response => {
                                this.$emit('reload')
                            }
            ...
            attendanceConfirm(){
                ...
                const params = {
                    ...
                    shift_working_hours: this.attendanceData.user.work_time_day * this.attendanceData.should_work_days,
                    shift_working_days: this.attendanceData.shift_count,
                    worked_days: this.attendanceData.workedday_count,
                    holiday_worked_days: this.attendanceData.holiday_count,
                    annual_leave: this.attendanceData.annual_leave/60,
                    condolence_leave: this.attendanceData.condolence_leave,
                    transfer_leave: this.attendanceData.transfer_leave,
                    worked_hours: this.attendanceData.worked_time,
                    worked_hours_no_over_time: this.attendanceData.worked_time - this.attendanceData.over_time,
                    over_time: this.attendanceData.month_over_time,
                    night_work_time: this.attendanceData.night_over_time,
                    stay_pay: this.attendanceData.month_stay_allowance_count,
                    move_pay: this.attendanceData.month_move_allowance_count
                }
                axios.post('attendance_confirm', params).then(
                    response => {
                        this.$emit('reload')
            ...
            attendanceCreate(){
                ...
                axios.post('/attendance_closed', params).then(
                    response => {
                        this.$emit('reload')
                        this.$emit('closeModal')
                    }
