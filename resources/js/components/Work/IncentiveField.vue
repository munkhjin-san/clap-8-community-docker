<template>
    <div class="report-field" style="background:inherit;">
        <p class="report-header" style="margin-bottom: 20px;">インセンティブ</p>
        <div v-for="(incentive, index) in model" :key="index" style="display:flex;gap:20px;background:inherit;margin-top:20px;align-items: center;">
            
            <ShortInput 
                name="expenses" 
                placeHolder="件" 
                type="number"
                v-model="incentive.count"
                customStyle="padding:0 10px; height:40px;width: 40px;"
            />
            <div v-if="!incentive.file" style="display:inline-block">
                <label for="incentiveFile" class="cursor-pointer" @click="currentIndex = index">                                       
                ...
                <input  accept="image/*" type="file" name="incentiveFile" id="incentiveFile" @change="addAttachment($event, currentIndex)" style="display: none;">
            </div>
            <div class="file-area-content" v-else style="margin:0">
                <div class="file-wrap" @click.stop style="min-width: auto;height:40px;min-height: auto;">   
                    <img
                        ...
                        :src=fileSrc(incentive.file)
                        @click="previewImage(incentive.file)"
                    />
                    <div @click.stop="fileUpCancel(incentive.file, index)" class="cancelButton">
            ...
            <div style="position: static;width: 30px;height: 30px; display:flex; cursor: pointer;" @click="addIncentive">
            ...
            <div v-if="model.length > 1 || incentive.id" style="position: static;width: 30px;height: 30px;display:flex; cursor: pointer;" @click="removeIncentive(index, incentive)">
...
<script setup>
    ...
    const removeIncentive = async(index, incentive) => {
        if(incentive?.id){
            const answer = await confirm('作業費を削除してもよろしいですか')
            if(!answer) return
            try {
                await axios.delete(`/work_incentive_delete?id=${incentive?.id}`)
            } catch (e) {
