<template>
    <div style="display:flex;gap:20px;background:inherit;margin-top:15px;align-items:center;flex-wrap:wrap;">
        <select class="optionPicker" v-model="type" name="costType" :disabled="locked">
            <option :key="index" v-for="(item, index) in costOptions" :value="item.value">{{ item.label }}</option>
        </select>
        <OptionSelector
            v-if="!hideDepartment"
            :options="workGroupAsOptions"
            name="departmentSelect"
            unit=""
            rules="required"
            v-model="department"
            :disabled="locked"
        />
        <select class="optionPicker" v-if="type == 4 || type == 1" name="transportType" v-model="transportType" :disabled="locked">
            <option :key="index" v-for="(item, index) in transportOptions" :value="item.value">
                {{ item.label }}
            </option>
        </select>
        <div class="flex gap-5 flex-wrap" v-if="type == 4 || type == 1">
            <input
                class="text-[var(--primary-color)] border border-[var(--primary-color)] border-solid px-[10px] h-[38px]"
                placeholder="出発"
                v-model="departurePlace"
                :disabled="locked"
            />
            <input
                class="text-[var(--primary-color)] border border-[var(--primary-color)] border-solid px-[10px] h-[38px]"
                placeholder="到着"
                v-model="arrivalPlace"
                :disabled="locked"
            />
        </div>
        
        <input
            name="content"
            placeholder="内容"
            type="text"
            style="padding:0 10px; height:38px;border:1px solid var(--primary-color);color:var(--primary-color);"
            v-model="content"
            :disabled="locked"
        />
        <div class="flex items-center gap-3">
            <div class="relative bg-inherit">
                <input
                    name="expenses"
                    placeholder="経費"
                    type="number"
                    style="padding: 0px 25px 0 10px; height:38px; width: 80px;border:1px solid var(--primary-color);color:var(--primary-color);"
                    v-model="expenses"
                    :disabled="locked"
                />
                <span style="position:absolute; height:100%; top:0; right: 5px; line-height: 38px;">円</span>
            </div>
            
            <button
                v-if="(type == 1 || type == 4) && expenses"
                type="button"
                class="round-trip-button"
                :class="{ 'round-trip-applied': roundTripApplied }"
                @click="applyRoundTrip"
                :disabled="locked"
            >
                往復計算
            </button>
        </div>
        <input
            name="merchant_name"
            placeholder="取引先"
            type="text"
            style="padding:0 10px; height:38px;border:1px solid var(--primary-color);color:var(--primary-color);"
            v-model="merchantName"
            :disabled="locked"
        />
        <input
            name="receipt_date"
            type="date"
            style="padding:0 10px; height:38px;border:1px solid var(--primary-color);"
            v-model="receiptDate"
            :class="[{ 'date-color': theme.dark }]"
            :disabled="locked"
        />

        <div v-if="!fileModel && !locked" style="display:inline-block; position: relative;">
            <div v-if="loading" class="l-spinner" style="background-color: var(--background-color);">
                <span style="min-width: 20px; height: 20px;" class="l-loader"></span>
            </div>
            <label :for="`sharedfile${fieldIndex}`" class="cursor-pointer">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="17" viewBox="0 0 27 32" style="fill: var(--primary-color)">
                    <path d="M25.954 7.013c-0.479-0.575-4.378-4.56-5.978-5.816-0.623-0.489-1.284-0.853-2.127-0.949-1.178-0.125-2.97-0.182-4.091-0.22-1.36-0.029-2.472-0.029-3.832-0.029-1.36 0.010-3.008 0.077-4.474 0.172-1.36 0.077-2.328 0.134-2.845 0.22-0.69 0.105-1.188 0.489-1.265 1.303-0.077 0.805-0.172 4.905-0.172 7.454 0.010 2.558 0.115 5.835 0.201 6.822 0.096 0.987 0.556 1.447 1.083 1.504 0.527 0.067 0.843-0.537 0.853-1.159 0.019-0.623 0.019-1.226 0.019-1.734s-0.048-2.913-0.019-5.432c0.029-2.098 0.086-4.206 0.192-6.304 0.010-0.134 0.115-0.24 0.249-0.24 0.92-0.029 1.849-0.048 2.778-0.058 1.341-0.019 2.683-0.019 4.024-0.010s2.683 0.029 4.024 0.058c0.987 0.019 1.983 0.048 2.96 0.086 0.153 0.010 0.268 0.134 0.268 0.287-0.010 0.901-0.019 3.612-0.019 3.612 0 0.546 0.010 1.083 0.019 1.629v0.010c0.010 0.546 0.45 0.987 0.996 0.987l1.705 0.019h1.705c0.441 0 1.428-0.019 1.926-0.029 0.153 0 0.287 0.125 0.297 0.278 0.048 1.399 0.067 2.807 0.077 4.216 0.010 1.878 0 3.756-0.029 5.634s-0.077 3.756-0.153 5.624c-0.067 1.514-0.144 3.037-0.268 4.532-0.019 0.201-0.182 0.355-0.383 0.364-1.418 0.038-2.778 0.067-4.302 0.077-1.648 0.010-6.266 0.010-8.163 0-1.964-0.010-5.365-0.029-7.042-0.086-0.125 0-0.24-0.153-0.259-0.278-0.058-0.374-0.105-0.834-0.163-1.389-0.067-0.623-0.469-1.092-1.035-1.025-0.45 0.048-0.824 0.45-0.891 1.198-0.067 0.738-0.019 1.619 0.067 2.213s0.441 1.016 1.198 1.14c1.006 0.163 5.72 0.249 8.057 0.268 2.347 0.019 6.275-0.019 8.259-0.019 1.974-0.010 3.286-0.019 4.857-0.182 1.121-0.115 1.408-0.747 1.552-1.715 0.24-1.667 0.345-3.325 0.469-4.982 0.134-1.887 0.24-3.775 0.326-5.672 0.086-1.887 0.144-3.784 0.192-5.672 0.029-1.428 0.038-3.21 0.019-4.235-0.010-0.948-0.287-1.782-0.862-2.472zM19.832 7.023c-0.019-0.537-0.077-2.060-0.096-2.692 0-0.096 0.105-0.144 0.182-0.086 0.537 0.489 2.491 2.271 3.152 2.874 0.077 0.067 0.029 0.192-0.077 0.182-0.719-0.029-2.434-0.086-2.98-0.105-0.096 0.010-0.182-0.077-0.182-0.172z"></path>
                    <path d="M18.405 25.61l2.050-6.189c0.029-0.086 0.048-0.182 0.048-0.268 0-0.45-0.383-0.843-0.881-0.843h-18.74c-0.24 0-0.46 0.096-0.623 0.249s-0.259 0.364-0.259 0.604v6.189c0 0.23 0.096 0.441 0.259 0.594s0.383 0.249 0.623 0.249h16.69c0.374-0.010 0.709-0.24 0.834-0.584zM22.41 11.89c0.019-0.383-0.278-0.719-0.671-0.738-1.284-0.067-2.568-0.096-3.842-0.115-0.642-0.010-1.284-0.029-1.926-0.029l-1.926-0.010-1.926 0.010-1.926 0.029c-1.284 0.019-2.568 0.038-3.842 0.086-0.374 0.019-0.69 0.316-0.699 0.699-0.010 0.402 0.297 0.738 0.699 0.757 1.284 0.048 2.568 0.067 3.842 0.086l1.926 0.019 1.926 0.010 1.926-0.010c0.642 0 1.284-0.019 1.926-0.029 1.284-0.019 2.568-0.048 3.842-0.115 0.364-0.010 0.651-0.287 0.671-0.652zM15.875 14.63c-0.527-0.010-1.054-0.029-1.581-0.029l-1.59-0.010-1.581 0.010-1.59 0.029c-1.054 0.019-2.117 0.038-3.171 0.086-0.374 0.019-0.68 0.316-0.69 0.699-0.019 0.402 0.297 0.738 0.69 0.757 1.054 0.048 2.117 0.067 3.171 0.086l1.59 0.029 1.581 0.010 1.59-0.010c0.527 0 1.054-0.019 1.581-0.029 1.054-0.019 2.117-0.048 3.171-0.115 0.345-0.019 0.632-0.297 0.661-0.661 0.019-0.383-0.268-0.719-0.661-0.738-1.054-0.057-2.117-0.086-3.171-0.115z"></path>
                </svg>
            </label>
            <input accept="image/*,application/pdf" type="file" :name="`sharedfile${fieldIndex}`" :id="`sharedfile${fieldIndex}`" @change="addAttachment($event)" style="display: none;">
        </div>

        <div v-if="fileModel" class="file-area-content" style="margin:0">
            <div v-if="currentFileType === 'image'" class="cost-file-container" @click.stop>
                <img
                    style="max-width:100%;margin:auto;max-height:100%;cursor: pointer;"
                    class="list-image-mobile"
                    :src="fileSrc(fileModel)"
                    @click="workFilePreview(fileModel, currentFileType, '/cdn/timecard_files')"
                />
                <div v-if="!locked" @click.stop="fileUpCancel()" class="cancelButton">
                    <svg @click.prevent version="1.1" xmlns="http://www.w3.org/2000/svg" width="7" height="7" viewBox="0 0 32 32" fill="var(--background-color)">
                        <path d="M31.165 28.569l-1.67-1.855-1.681-1.841-6.777-7.318c-0.362-0.387-0.964-1.006-1.363-1.412-0.227-0.23-0.227-0.594-0.001-0.826 0.397-0.408 0.993-1.023 1.355-1.409 1.133-1.215 2.25-2.446 3.378-3.667l3.375-3.674c1.12-1.227 2.233-2.463 3.335-3.709 0.569-0.64 0.583-1.621 0-2.278-0.629-0.712-1.715-0.779-2.426-0.15-1.247 1.103-2.482 2.218-3.711 3.338l-3.672 3.374c-1.222 1.128-2.453 2.246-3.669 3.378-0.49 0.456-0.967 0.925-1.447 1.394-0.211 0.206-0.551 0.206-0.765 0-0.48-0.469-0.957-0.938-1.448-1.394-1.213-1.13-2.443-2.248-3.665-3.375l-3.672-3.374c-1.23-1.121-2.465-2.234-3.711-3.338-0.641-0.566-1.621-0.582-2.279 0-0.712 0.63-0.779 1.717-0.149 2.428 1.103 1.247 2.218 2.482 3.336 3.709l3.375 3.674c1.127 1.222 2.244 2.453 3.378 3.667 0.36 0.385 0.957 1.002 1.354 1.409 0.227 0.232 0.225 0.597-0.001 0.826-0.401 0.406-1.002 1.024-1.363 1.412l-3.389 3.655-3.388 3.661-1.682 1.841-1.668 1.855c-0.6 0.669-0.615 1.707 0 2.392 0.661 0.732 1.789 0.792 2.522 0.131l1.855-1.667 1.841-1.682 7.318-6.776c0.487-0.455 0.959-0.922 1.432-1.389 0.214-0.209 0.557-0.209 0.769 0 0.476 0.466 0.949 0.934 1.433 1.389l7.318 6.776 1.841 1.682 1.855 1.667c0.671 0.602 1.707 0.618 2.392 0 0.736-0.659 0.796-1.789 0.135-2.522z"></path>
                    </svg>
                </div>
            </div>
            <div v-else class="cost-file-container" @click.stop>
                <div style="position:relative;" @click="workFilePreview(fileModel, currentFileType, '/cdn/timecard_files')">
                    <FileIcon ext="pdf" />
                </div>
                <div v-if="!locked" @click.stop="fileUpCancel()" class="cancelButton">
                    <svg @click.prevent version="1.1" xmlns="http://www.w3.org/2000/svg" width="7" height="7" viewBox="0 0 32 32" fill="var(--background-color)">
                        <path d="M31.165 28.569l-1.67-1.855-1.681-1.841-6.777-7.318c-0.362-0.387-0.964-1.006-1.363-1.412-0.227-0.23-0.227-0.594-0.001-0.826 0.397-0.408 0.993-1.023 1.355-1.409 1.133-1.215 2.25-2.446 3.378-3.667l3.375-3.674c1.12-1.227 2.233-2.463 3.335-3.709 0.569-0.64 0.583-1.621 0-2.278-0.629-0.712-1.715-0.779-2.426-0.15-1.247 1.103-2.482 2.218-3.711 3.338l-3.672 3.374c-1.222 1.128-2.453 2.246-3.669 3.378-0.49 0.456-0.967 0.925-1.447 1.394-0.211 0.206-0.551 0.206-0.765 0-0.48-0.469-0.957-0.938-1.448-1.394-1.213-1.13-2.443-2.248-3.665-3.375l-3.672-3.374c-1.23-1.121-2.465-2.234-3.711-3.338-0.641-0.566-1.621-0.582-2.279 0-0.712 0.63-0.779 1.717-0.149 2.428 1.103 1.247 2.218 2.482 3.336 3.709l3.375 3.674c1.127 1.222 2.244 2.453 3.378 3.667 0.36 0.385 0.957 1.002 1.354 1.409 0.227 0.232 0.225 0.597-0.001 0.826-0.401 0.406-1.002 1.024-1.363 1.412l-3.389 3.655-3.388 3.661-1.682 1.841-1.668 1.855c-0.6 0.669-0.615 1.707 0 2.392 0.661 0.732 1.789 0.792 2.522 0.131l1.855-1.667 1.841-1.682 7.318-6.776c0.487-0.455 0.959-0.922 1.432-1.389 0.214-0.209 0.557-0.209 0.769 0 0.476 0.466 0.949 0.934 1.433 1.389l7.318 6.776 1.841 1.682 1.855 1.667c0.671 0.602 1.707 0.618 2.392 0 0.736-0.659 0.796-1.789 0.135-2.522z"></path>
                    </svg>
                </div>
            </div>
        </div>

        <div v-if="fileModel" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
            <button v-if="!locked" type="button" class="ocr-action-button" @click="runOcr" :disabled="ocrLoading">
                {{ ocrLoading ? 'OCR中...' : 'OCR' }}
            </button>
            <span v-if="fileOriginalName" style="font-size:12px;">{{ fileOriginalName }}</span>
        </div>

        <div class="op-button-container cost-field-actions" v-if="!locked">
            <button
                type="button"
                class="project-time-action-button"
                title="経費を追加"
                aria-label="経費を追加"
                @click="emit('addCostField')"
            >
                ＋
            </button>
            <button
                type="button"
                class="project-time-action-button"
                title="経費を削除"
                aria-label="経費を削除"
                @click="emit('removeCostField', fieldIndex)"
            >
                −
            </button>
        </div>
    </div>

    <div v-if="multipleReceiptsDetected" class="ocr-multi-receipt-warning">
        <span class="ocr-multi-receipt-icon">⚠️</span>
        <div>
            <strong>複数の領収書が検出されました</strong><br>
            <span>1枚ずつ別々にアップロードしてください。OCR候補は1枚目の内容のみです。</span>
        </div>
    </div>

    <div v-if="hasOcrSuggestions" class="ocr-suggestion-card">
        <div class="ocr-suggestion-header">
            <strong>OCR候補</strong>
            <button v-if="!locked" type="button" class="ocr-action-button" @click="applyAllOcr">すべて反映</button>
        </div>
        <div class="ocr-suggestion-grid">
            <div v-for="suggestion in suggestionRows" :key="suggestion.key" class="ocr-suggestion-item">
                <span class="ocr-suggestion-label">{{ suggestion.label }}</span>
                <span class="ocr-suggestion-value">{{ suggestion.value || '未検出' }}</span>
                <button
                    v-if="!locked && suggestion.value"
                    type="button"
                    class="ocr-apply-button"
                    @click="applyOcrFields([suggestion.key])"
                >
                    反映
                </button>
            </div>
        </div>
    </div>
</template>
<script setup>
import { computed, ref, watch } from 'vue';
import OptionSelector from '../Form/OptionSelector.vue';
import FileIcon from '../Board/Mixed/FileIcon.vue';
import { filePreviewTypeFromPath, workFilePreview } from '../../utils/workApi';
import { useDialog } from '@/composables/dialog';
import { useApi } from '@/composables/api';
import { useTheme } from '@/store/theme';
import { DateTime } from 'luxon';

const department = defineModel('department')
const content = defineModel('content')
const type = defineModel('type')
const transportType = defineModel('transport_type')
const departurePlace = defineModel('departure_place')
const arrivalPlace = defineModel('arrival_place')
const expenses = defineModel('expenses')
const fileModel = defineModel('file_path')
const receiptFileId = defineModel('receipt_file_id')
const draftUuid = defineModel('draft_uuid')
const merchantName = defineModel('merchant_name')
const receiptDate = defineModel('receipt_date')
const currency = defineModel('currency')
const receiptSourceType = defineModel('receipt_source_type')
const fileOriginalName = defineModel('file_original_name')
const fileMimeType = defineModel('file_mime_type')
const fileSizeBytes = defineModel('file_size_bytes')
const fileSha256 = defineModel('file_sha256')
const fileUploadedAt = defineModel('file_uploaded_at')
const scanDpi = defineModel('scan_dpi')
const scanColorDepth = defineModel('scan_color_depth')
const scanColorMode = defineModel('scan_color_mode')
const documentSize = defineModel('document_size')
const imageWidthPx = defineModel('image_width_px')
const imageHeightPx = defineModel('image_height_px')
const ocrRunId = defineModel('ocr_run_id')
const ocrAppliedFields = defineModel('ocr_applied_fields')
const roundTripApplied = ref(false)
let skipExpensesWatch = false
const props = defineProps({
    workGroupAsOptions: Array,
    fieldIndex: Number,
    isRegistered: Boolean,
    subjectUserId: Number,
    timecardRecordId: Number,
    timecardCostRecordId: Number,
    locked: Boolean,
    hideDepartment: Boolean,
})

const emit = defineEmits(['addCostField', 'removeCostField', 'removeFile'])

const loading = ref(false)
const ocrLoading = ref(false)
const ocrSuggestions = ref(null)
const multipleReceiptsDetected = ref(false)
const theme = useTheme()
const api = useApi()
const { ping } = useDialog()

const currentFileType = computed(() => {
    return filePreviewTypeFromPath(fileModel.value, fileMimeType.value)
})

const hasOcrSuggestions = computed(() => {
    return suggestionRows.value.some((row) => Boolean(row.value))
})

const supportsTransportOcr = computed(() => [1, 4].includes(Number(type.value)))



const suggestionRows = computed(() => {
    const source = ocrSuggestions.value ?? {}
    const rows = [
        { key: 'merchant_name', label: supportsTransportOcr.value && source.transport ? '交通内容' : '取引先', value: supportsTransportOcr.value ? (source.transport || source.merchant_name || '') : (source.merchant_name ?? '') },
        { key: 'receipt_date', label: '領収書日付', value: (source.receipt_date ? normalizeReceiptDate(source.receipt_date) : '') },
        { key: 'amount', label: '金額', value: source.amount ?? '' },
        { key: 'currency', label: '通貨', value: source.currency ?? '' },
        { key: 'receipt_source_type', label: '保存種別', value: receiptSourceTypeLabelMap[source.receipt_source_type] ?? source.receipt_source_type ?? '' },
    ]

    if (supportsTransportOcr.value) {
        rows.splice(2, 0,
            { key: 'transport_type', label: '交通手段', value: transportLabelMap[source.transport_type] ?? '' },
            { key: 'departure_place', label: '出発', value: source.departure_place ?? '' },
            { key: 'arrival_place', label: '到着', value: source.arrival_place ?? '' },
        )
    }

    return rows
})
const transportOptions = [
    { label: '電車のみ', value: 1 },
    { label: '電車・バス', value: 2 },
    { label: 'タクシー', value: 3 },
    { label: '飛行機', value: 4 },
    { label: 'その他', value: 5 },
]
const receiptSourceTypeLabelMap = {
    paper_scan: 'スキャナ保存',
    electronic: '電子取引データ',
}
const costOptions = computed(() => {
    const registeredOptions = [
        { label: '交通費', value: 1 },
        { label: '通信費', value: 2 },
        { label: '宿泊費', value: 3 },
    ]
    const fulltimeOptions = [
        { label: '旅費交通費', value: 4 },
        { label: '通信費', value: 2 },
        { label: '消耗品費', value: 5 },
        { label: '交際費', value: 6 },
        { label: '支払手数料', value: 7 },
        { label: '福利厚生費', value: 8 },
    ]

    return props.isRegistered ? registeredOptions : fulltimeOptions
})

const transportLabelMap = {
    1: '電車のみ',
    2: '電車・バス',
    3: 'タクシー',
    4: '飛行機',
    5: 'その他',
}

const buildTransportContent = () => {
    const route = [departurePlace.value, arrivalPlace.value].filter(Boolean).join(' → ')
    const transportLabel = transportLabelMap[transportType.value] ?? ''
    const roundTripSuffix = roundTripApplied.value ? '（往復）' : ''

    if (!route) {
        return ''
    }

    if (transportLabel) {
        return `${transportLabel} / ${route}${roundTripSuffix}`
    }

    return `${route}${roundTripSuffix}`
}

const fileSrc = (file) => `/cdn/timecard_files/${file}`

const addAttachment = (event) => {
    if (event.target.files && event.target.files.length) {
        uploadStart(event.target.files[0])
    }
}

const uploadStart = async(file) => {
    if (!file) return

    const formData = new FormData()
    formData.append('file', file)
    formData.append('draft_uuid', draftUuid.value)
    if (props.subjectUserId) {
        formData.append('subject_user_id', String(props.subjectUserId))
    }
    if (props.timecardRecordId) {
        formData.append('timecard_record_id', String(props.timecardRecordId))
    }

    const response = await api.post('/work_file_upload', formData, {
        loadingRef: loading,
    })
    if (!response) return

    fileModel.value = response.file_path
    receiptFileId.value = response.receipt_file_id
    fileOriginalName.value = response.file_original_name
    fileMimeType.value = response.file_mime_type
    fileSizeBytes.value = response.file_size_bytes
    fileSha256.value = response.file_sha256
    fileUploadedAt.value = response.file_uploaded_at
    scanDpi.value = response.scan_dpi
    scanColorDepth.value = response.scan_color_depth
    scanColorMode.value = response.scan_color_mode
    documentSize.value = response.document_size
    imageWidthPx.value = response.image_width_px
    imageHeightPx.value = response.image_height_px
    ocrRunId.value = null
    ocrAppliedFields.value = []
    ocrSuggestions.value = null
    multipleReceiptsDetected.value = false
}

const fileUpCancel = async() => {
    ocrSuggestions.value = null
    multipleReceiptsDetected.value = false
    emit('removeFile', props.fieldIndex)
}

const runOcr = async() => {
    if (!fileModel.value) {
        ping('領収書ファイルをアップロードしてください。')
        return
    }

    const response = await api.post('/work_receipt_ocr', {
        draft_uuid: draftUuid.value,
        file_path: fileModel.value,
        expense_type: type.value,
        subject_user_id: props.subjectUserId,
        timecard_record_id: props.timecardRecordId,
        timecard_cost_record_id: props.timecardCostRecordId,
    }, {
        loadingRef: ocrLoading,
    })
    if (!response) return

    ocrRunId.value = response.ocr_run_id
    ocrSuggestions.value = response.suggestions ?? {}
    multipleReceiptsDetected.value = response.multiple_receipts_detected === true
}

const applyAllOcr = () => {
    const fields = ['merchant_name', 'receipt_date', 'amount', 'currency', 'receipt_source_type']
    if (supportsTransportOcr.value) {
        fields.splice(2, 0, 'transport_type', 'departure_place', 'arrival_place')
    }
    applyOcrFields(fields)
}

const applyOcrFields = (fields) => {
    const source = ocrSuggestions.value ?? {}
    if (!fields.length) return

    if (fields.includes('merchant_name')) {
        if (supportsTransportOcr.value && source.transport) {
            merchantName.value = source.transport
        } else if (source.merchant_name) {
            merchantName.value = source.merchant_name
        }
    }
    if (fields.includes('receipt_date') && source.receipt_date) {
        receiptDate.value = normalizeReceiptDate(source.receipt_date)
    }
    if (fields.includes('transport_type') && source.transport_type) {
        transportType.value = Number(source.transport_type)
    }
    if (fields.includes('departure_place') && source.departure_place) {
        departurePlace.value = source.departure_place
    }
    if (fields.includes('arrival_place') && source.arrival_place) {
        arrivalPlace.value = source.arrival_place
    }
    if (fields.includes('amount') && source.amount) {
        expenses.value = source.amount
    }
    if (fields.includes('currency') && source.currency) {
        currency.value = source.currency
    }
    if (fields.includes('receipt_source_type') && source.receipt_source_type) {
        receiptSourceType.value = source.receipt_source_type
    }

    const applied = new Set([...(ocrAppliedFields.value ?? []), ...fields])
    ocrAppliedFields.value = Array.from(applied)
}
const normalizeReceiptDate = (date) => {
    const todayYear = DateTime.now().year
    const [year, month, day] = date.split('-')

    return `${todayYear}-${month}-${day}`
}
watch(fileModel, (value) => {
    if (!value) {
        ocrSuggestions.value = null
        multipleReceiptsDetected.value = false
    }
})

watch(
    [type, transportType, departurePlace, arrivalPlace],
    ([nextType]) => {
        if (![1, 4].includes(Number(nextType))) {
            return
        }

        content.value = buildTransportContent()
    },
    { immediate: true }
)

watch(type, (nextType, previousType) => {
    if ([1, 4].includes(Number(nextType))) {
        if (!transportType.value) {
            transportType.value = 1
        }
        content.value = buildTransportContent()
        return
    }

    if ([1, 4].includes(Number(previousType))) {
        transportType.value = null
        departurePlace.value = ''
        arrivalPlace.value = ''
        content.value = ''
    }
})
const applyRoundTrip = () => {
    if (roundTripApplied.value) {
        ping('往復計算済みです。金額を変更すると再計算できます。')
        return
    }
    if (!expenses.value) return
    skipExpensesWatch = true
    expenses.value = String(Number(expenses.value) * 2)
    roundTripApplied.value = true
}

watch(expenses, () => {
    if (skipExpensesWatch) {
        skipExpensesWatch = false
        return
    }
    roundTripApplied.value = false
})

watch(roundTripApplied, () => {
    if ([1, 4].includes(Number(type.value))) {
        content.value = buildTransportContent()
    }
})
</script>
<style scoped>
    .l-loader:before {
        border: 3px solid var(--primary-color);
    }

    .ocr-multi-receipt-warning {
        margin-top: 10px;
        padding: 10px 14px;
        display: flex;
        gap: 10px;
        align-items: flex-start;
        background: #fff8e1;
        border: 1px solid #f0b429;
        border-radius: 4px;
        font-size: 12px;
        line-height: 1.6;
        color: #7a5200;
    }

    .ocr-multi-receipt-icon {
        font-size: 16px;
        flex-shrink: 0;
        margin-top: 1px;
    }

    .ocr-suggestion-card {
        margin-top: 10px;
        padding: 12px;
        border: 1px solid var(--primary-color);
    }

    .ocr-suggestion-header {
        display: flex;
        justify-content: space-between;
        align-items: center;
        gap: 12px;
        margin-bottom: 10px;
    }

    .ocr-suggestion-grid {
        display: grid;
        gap: 8px;
    }

    .ocr-suggestion-item {
        display: flex;
        align-items: center;
        gap: 12px;
        flex-wrap: wrap;
        font-size: 12px;
    }

    .ocr-suggestion-label {
        min-width: 90px;
        font-weight: 700;
    }

    .ocr-suggestion-value {
        min-width: 120px;
    }

    .ocr-action-button,
    .ocr-apply-button {
        padding: 6px 10px;
        color: #fff;
        background: var(--primary-button);
        font-size: 12px;
    }
    .round-trip-button {
        padding: 0 12px;
        height: 30px;
        color: var(--primary-color);
        background: var(--bg3);
        border: solid thin var(--bg3);
        font-size: 14px;
        white-space: nowrap;
        cursor: pointer;
    }
    .round-trip-button.round-trip-applied {
        background: var(--primary-button);
        color: #fff;
        border-color: var(--primary-button);
    }
    .round-trip-button:disabled {
        opacity: 0.5;
        cursor: default;
    }
    .cost-field-actions {
        align-items: center;
        gap: 6px;
    }
</style>
