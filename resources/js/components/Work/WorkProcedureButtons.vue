    ...
    <div class="work-modal" @mousedown="emit('closeModal')">
        <div class="work-modal-inner overstyle" @mousedown.stop>
            <div class="recordFormTitle" style="z-index: 26;gap:30px;">
                <p style="font-size: 18px;">{{ modalTitle }}</p>
            ...
            <div class="work-procedure-button-list">
                ...
                <LoaderButton
                    ...
                    v-if="canApproveSelectedProjectSegment"
                    content="日報差戻"
                    @triggered="emitProjectSegmentAction('rejectProjectSegment', selectedSegment)"
                />
                <LoaderButton
                    style="margin:0"
                    v-if="canCancelSelectedProjectSegment"
                    content="承認取消"
                    @triggered="emitProjectSegmentAction('cancelProjectSegment', selectedSegment)"
                />
                <LoaderButton
                    style="margin:0"
                ...
                <LoaderButton
                    style="margin:0"
                    v-if="canEditDailyReport"
                    content="日報編集"
                    @triggered="edit(item), emit('closeModal')"
                />
                <LoaderButton
                    style="margin:0"
                    v-if="canDeleteDailyReport"
                    content="日報削除"
...
import { useDashboardStore } from '@/store/dashboard';
    ...
    const modalTitle = computed(() => {
        return [formatedDay.value, props.selectedSegment ? projectSegmentName(props.selectedSegment) : '']
            .filter(Boolean)
            .join(' ')
    })
    const emitProjectSegmentAction = (eventName, segment) => {
        emit(eventName, segment, props.item)
        emit('closeModal')
    }
...
<style scoped>
    .overstyle{
        max-width: min(720px, 30vw);
        width: fit-content;
        height:fit-content;
    }
    .work-procedure-button-list {
        display: flex;
        gap: 30px;
        flex-wrap: wrap;
        margin-top: 10px;
