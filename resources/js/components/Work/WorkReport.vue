        ...
        <div class="work-modal-inner" @mousedown.stop>
            <div class="recordFormTitle">
                ...
                <div @click="$emit('closeModal')" class="cursor-pointer" style="display:flex;align-items: center;margin: auto 0 auto auto;">
                    <button class="work-delete-button" @click.stop="deleteTimeCard" v-if="!createReport">削除する</button>
            ...
            <div class="report-wrapper">
                ...
                <div class="report-field" v-if="editTime">
                    ...
                    <div class="report-input-time">
                        <div>
                            <input class="taskDateTimePicker" :class="{'clock-color' : $store.state.dark == true }" type="time" v-model="editStartTime" step="900">
                        </div>
                        <div class="between-line">～</div>
                        <div>
                            <input class="taskDateTimePicker" :class="{'clock-color' : $store.state.dark == true }" type="time" v-model="editEndTime" step="900">
                        </div>
                ...
                <div class="report-button-wrapper">
                    <button class="r-button cursor-pointer" :disabled="loading" style="margin-top:30px;" @click="saveTimeCard(0)">
                        保存する
                    </button>
                    <button class="r-button cursor-pointer" :disabled="loading" style="margin-top:30px;" @click="saveTimeCard(1)">
                        申請する
                    </button>
                </div>
            </div>
        ...
        methods: {
            ...
            deleteTimeCard(){
                ...
                emitter.emit('setToast', {
                    active: true,  
                    type: 'info', 
                    content: `${this.formatedDay}の日報を削除しますか。`,
                    closeButton: false, 
                    autoClose: false,
                    answers: [this.$t('confirmToAction'),this.$t('cancelToAction')],
                    channel: uniqueChannell
                ...
                emitter.on(uniqueChannell, (data) => { 
                    if(data.answer === this.$t('confirmToAction')){
                        ...
                        axios.post('/delete_time_card', params).then(
                            response => {
                                this.$emit('reload')
                            }
                        ).catch(function (error) {
