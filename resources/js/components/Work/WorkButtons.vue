<template>
    <div class="workButtons-wrapper">
        ...
        <Transition name="modalFade">
            <div v-if="$store.state.menu.id == 98 && $store.state.menu.name == 'workMemberSelector'" id="workMemberSelector" class="workMemberSelector">
                <div class="searchBarInner" style="height: 40px;margin:0 15px;min-width:240px;"> 
                    <input @input="setKeyord" class="searchBarArea searchInputArea memberSearch" id="workMemberSearch" placeholder="ユーザー検索" type="text" style="margin: auto 0 auto auto;width:100%;background:#fff"/>
                    ...
                    <div>
                        ...
                        <div :key="user.id" v-for="user in searchUsers" style="padding:0 15px;display:flex;">                                
                            <label class="work-member-check" style="align-self: center;padding-left: 30px;padding-bottom: 0;margin-bottom: 0;">
                                <input @change="update" :checked="checked(user.id)" :value="user.id" name="memberCheckBox" type="checkbox">
...
<script>
    export default{
        ...
        data(){
            return{
                buttons: [
                    { label: '勤怠予定', action: 'selectShift'},
                    { label: '勤怠確定', action: 'confirmAttendance'},
                    { label: 'メンバー', action: 'selectMember'},
                    { label: '今日', action: 'jumpToToday'},
                    { label: '集計', action: 'jumpToTotal'}
                ],
                loading: false,
                idArray: this.usersCheckArray,
                keywords: null,
                allSelected: false
            }
        },
        computed: {
            searchUsers(){
                if(this.keywords && Array.isArray(this.workGroups)){
                    let lowSearch = this.keywords.toLowerCase()
                    return this.workGroups.filter(user => 
        ...
        methods: {
            ...
            update(){
                ...
                this.$emit('selectedUsers', this.idArray)
                this.keywords = null
            },
            setKeyord(){
            ...
            selectAll(){
                ...
                if(this.allSelected == true){
                    const ids = this.searchUsers.map(ob => ob.id);
                    this.idArray = ids
                    this.$emit('selectedUsers', this.idArray)
                }else{
                    this.idArray = [this.auth_user.id]
                    this.$emit('selectedUsers', this.idArray)
                }
            }
        }
