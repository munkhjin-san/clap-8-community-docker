<template>
    <div class="sub-task-item">         
        <div :style="{ width: `${spacerWidth}%` }" class="width-smooth"></div>
        <div class="sub-task-inner task-card-inner width-smooth" :id="`gantt-sub-${task.id}`" :style="{ width: `${taskWidth}%`, background: background, color: color }"> 
                ...
                <div class="flex flex-col gap-[5px] h-full w-full">
                    ...
                    <div v-if="errorMessages.length" class="flex text-[tomato] text-[11px] my-[5px]">
                        ...
                        <div class="flex flex-col gap-[5px]">
                            <p v-for="message in errorMessages" style="text-wrap: auto;" class="ml-[5px] overflow-hidden leading-[1.2]">{{ message }}</p>
                        </div>  
                    </div>  
                    <GanttButton viewType="button" v-if="isExecutor" :status="isExecutor.pivot.progress_flag" :loading="updating.status" @action="(flag) => updateStatus(flag)"/>
                    <div class="flex items-center absolute right-[5px] top-[10px] ">
                        <div @click="emit('setCommentingTaskId', Number(task.id))" class="flex items-center gap-[2px] cursor-pointer mr-[5px]">
                            <svg :fill="color" height="15" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 32">
                            ...
                            <div v-if="task.comments.length">{{ task.comments.length }}</div>
                            <!-- <span v-if="task.unread_comments" class="text-[#fff] bg-[tomato] min-w-[12px] p-px h-[12px] text-center text-[10px] rounded-[50%] leading-[12px] [text-indent:1px]">
                                {{ task.unread_comments > 9 ? '9+' : task.unread_comments }}
                            </span> -->
                        </div>  
...
const spacerWidth = computed(() => {
    if(props.from == 'board') return 0
    const total = props.interval.count('days')
    const offset = props.interval.start ? DateTime.fromISO(props.task.start_at).diff(props.interval.start, 'days').as('days') : 0; 
    const startPercent = Math.floor(offset / total * 100)
    const unit = startPercent
    return unit
})

const taskWidth = computed(() => {
    if(props.from == 'board') return 100
    const totalDuration = props.interval.length('days')
    const taskDuration = DateTime.fromISO(props.task.end_at).diff(DateTime.fromISO(props.task.start_at), 'days').as('days') 
    const percent = Math.ceil(taskDuration / totalDuration * 100)
    return percent
})
