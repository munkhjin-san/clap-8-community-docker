                ...
                <div class="flex flex-col gap-[5px] h-full w-full">
                    ...
                    <div class="flex items-center absolute right-[5px] top-[10px] ">
                        <div @click="emit('setCommentingTaskId', Number(task.id))" class="flex items-center gap-[2px] cursor-pointer mr-[5px]">
                            ...
                            <span v-if="commentBadge?.[task.id]" class="text-[#fff] bg-[tomato] min-w-[12px] p-px h-[12px] text-center text-[10px] rounded-[50%] leading-[12px] [text-indent:1px]">
                                {{ commentBadge[task.id] > 9 ? '9+' : commentBadge[task.id] }}
                            </span>
                        </div>  
...
const commentBadge = computed(() => {
    return badge.project?.grouped_task_badge || {}
})
