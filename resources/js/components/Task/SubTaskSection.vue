<template>
    <div style="margin-bottom: 20px;background: var(--bg3);padding: 15px;">
        <div style="display: flex;align-items: center;gap: 15px;">
            <ShortInput 
                style="flex: 1;"
                name="recordDateStart" 
                ref="subTaskContentRef"
                rules="required"
                type="text"
                v-model="remarks"
                :place-holder="`サブタスク${subTaskIndex ?  subTaskIndex+ 1 : ''}メモ`"
            /> 
            <div v-if="multiple && subTaskIndex !== undefined" @click="emit('remove', subTaskIndex)" style="height: 30px;width: 30px;cursor: pointer;align-items: center;justify-content: center;display: flex;">
        ...
        <div>
            <div style="font-size: 12px;margin: 15px 0;">担当者設定</div>
            <MemberSelector 
                style="margin-top: 15px;"
                placeHolder="メンバー選択"
                name="subTaskMembers"
                ref="subTaskMembersRef"
                rules="required"
                :multiple="true"
                :closeOnSelect="false"
                v-model="executors"   
        ...
        <div class="si-box">
            <p class="form-lbl" style="white-space: nowrap;font-size: 14px;">{{ `サブタスク${subTaskIndex ? subTaskIndex + 1 : ''}期限` }}</p>
