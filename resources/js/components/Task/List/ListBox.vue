    ...
    <div :class="boxClass">
        <div class="flex justify-between">            
            <div :id="'task_box_' + item.id" class="task-box-inner" :style="{backgroundColor: taskColor.mycolor, color: taskColor.color, position: 'relative', cursor: 'pointer'}">
                <div class="flex items-center justify-between w-full relative" :style="{ marginTop: responsive.mobile ? '0' : '5px'}">
                    ...
                    <div class="flex gap-[5px] items-center">
                        ...
                        <ItemMenu 
                            v-if="completeButtonFilter && isBoard"
                ...
                <div v-if="item.end_at" style="margin-top: 10px;">
                    <div v-if="!isBoard" :style="{fontSize: '12px', color: dateColor}">{{ `${DateTime.fromISO(item.start_at!).toLocaleString()} ~ ${DateTime.fromISO(item.end_at!).toLocaleString()}` }}</div>
                    <div v-else :style="{fontSize: '12px', color: dateColor}">{{ dateDetail(item.end_at!) }}</div>                         
                </div>
