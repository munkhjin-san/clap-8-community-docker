<template>
    <div class="chat-box">
        <div class="w-full h-full relative">                     
            <div class="h-[40px] flex items-center chat-header bg-[var(--primary-color)] text-[var(--background-color)]">
                <p class="ml-[20px]">AIチャット</p>
                <div @click="emit('close')" class="flex items-center justify-center min-w-[30px] h-[30px] cursor-pointer ml-auto">
                    <CloseIcon size="10" fill="var(--background-color)"/>
                </div>
            </div>
            <div></div>
            <div class="h-[calc(100%-40px)] relative msg-container">         
                ...
                <div class="h-[calc(100%-110px)] overflow-y-auto bg-[var(--message-background)]" ref="chatBody">
                    <div v-for="(msg, index) in conversationItemsList" :key="index" :class="msg.role === 'user' ? 'text-right' : 'text-left'">
                        <!-- <div class="max-w-[80%] inline-block p-[20px] leading-normal m-[20px] rounded-lg bg-[var(--bg3)] text-[var(--primary-color)] text-[14px] whitespace-pre-wrap">                        
                            <p class="" v-html="msg.message"></p>
                            <p style="border-top: solid thin gray;" class="mt-[20px] pt-[20px]" v-if="msg.keywords">
                                <strong>キーワード:</strong> {{ msg.keywords.join(', ') }}
                            </p>
                            <p class="mt-[20px]" v-if="msg.source">
                                <strong>参考資料:</strong>
                                <ul class="list-disc list-inside">
                                    <li v-for="(src, srcIndex) in msg.source" :key="srcIndex">{{ src }}</li>
                                </ul>
                            </p>
                        </div> -->
                ...
                <div class="flex items-center p-[10px] w-[calc(100%-20px)] h-[30px] bg-[var(--message-background)]">
