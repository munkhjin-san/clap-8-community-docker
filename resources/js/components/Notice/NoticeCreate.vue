    ...
    <div class="overlay" @mousedown="$emit('close', false)">                         
        <div class="chatCreate scrollable" @mousedown.stop>     
            <div class="recordFormTitle" style="display:flex">
                <p>{{ editTarget ? `お知らせを編集する` : `お知らせを作成する`}}</p>
                <div class="m-close-button" @click="$emit('close', false)" style="position:unset; margin:auto 0 auto auto">
            ...
            <div class="si-box">                   
                <FormLongText
                    ...
                    rules="required"
                    label="本文"
                    @setValue="val => body = val"
                />                    
            </div>
            <div class="si-box">
                <NoticeFileUploader
                    :initialValue="existFiles"
                    @updated="val => existFiles = val"
                    @uploaded="val => uploadFiles = val"
                    path="/notice_files"
                />
            </div>
            <div class="si-box">
...
export default{
    ...
    data(){
        return{
            ...
            body: this.editTarget && this.editTarget.body ? this.editTarget.body : "",
            existFiles: this.editTarget && this.editTarget.files ? this.editTarget.files : [],
            uploadFiles: [],
            processing: false
        }
    },
    ...
    methods:{
        async validation(){               
            ...
            try {                    
                let result = true
                let checkRef = ['noticeTitle', 'noticeContent']
        ...
        async checkConfirm(){
            ...
            const params = {
                ...
                title: this.title,
                body: this.body,
                new_files: this.uploadFiles.length ? this.uploadFiles : [],
                exist_files: this.existFiles.length ? this.existFiles.map(ob => ob.id) : []
            }
            axios.post('/notice_add_record',params)
            .then(response =>  {
