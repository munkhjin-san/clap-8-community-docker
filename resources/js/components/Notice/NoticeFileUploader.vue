<template>
    ...
    <div :class="['formFileUploadArea', {dropHereArea: entered}]"
        ...
        <div class="file-area-content" v-if="existFiles" style="padding: 20px 10px 10px 10px;margin:0">
            <div class="file-wrap" v-for="(file, index) in existFiles" @click.stop>   
                    ...
                    <div class="flex-centered">             
                        <div v-if="file.mime_type == 'image'" style="max-width:65px;height:40px;display: flex;min-width: 40px;min-height: 40px;">  
                            <img
                                ...
                                :src="`${$store.state.baseLocation}/notice_files/${file.id}_${file.user_id}_${file.record_id}.${file.extension}`" 
                            />
                        </div>
                ...
                <div @click.stop="removeExist(file)" class="cancelButton">
                    ...
                    <div class="flex-centered">             
                        <div v-if="file.mime_type == 'image'" style="max-width:65px;height:40px;display: flex;min-width: 40px;min-height: 40px;">  
                            <img
                                ...
                                class="list-image-mobile" 
                                :src="`/notice_temp/${file.id}.${file.extension}`" 
                            />
                        </div>
                        <div v-if="file.mime_type !== 'image'" style="position:relative;">
                            <FileIcon :ext="file.extension"/>
...
export default{
    props: ['initialValue', 'path', 'editTarget'],
    emits: ['updated', 'uploaded'],
    data(){
        return{
            entered: false,
            uploadingProgress: 0,
            existFiles: this.initialValue ? this.initialValue : [],
            uploadFiles: []
        }
    },
    components:{FileIcon},
    watch:{
        ...
        uploadFiles(after){
            this.$emit('uploaded', after)
        }
    },
    methods:{
        removeAttachment(file){
            axios.post('/notice_delete_file', {list: [file.id], path: this.path})
            .then(response =>{                                       
                const index = this.uploadFiles.findIndex(item => item.id === file.id);
                if (index !== -1) {
                    this.uploadFiles.splice(index, 1);
                }
                this.$emit('uploaded', this.uploadFiles)
            })
        },
        removeExist(file){  
            const index = this.existFiles.findIndex(item => item.id === file.id);
            if (index !== -1) {
                this.existFiles.splice(index, 1);
            }
            this.$emit('updated', this.existFiles)
        },
        dropFile(){
            this.entered = false
        },
        ...
        uploadStart(files){
            if(files && files.length){
                ...
                axios.post('/notice_file_upload', formData , { onUploadProgress: (e) => this.uploadingProgress = Math.floor((e.loaded * 100) / e.total) } )
                .then(response =>{                                       
