<template>
    <div class="routeposition" style="z-index: 24;">
        ...
        <div class="post-header">
            <div @click="router.go(-1)" class="cursor-pointer" style="display: flex;align-items: center;height: 50px;position: sticky;top: 0;background: var(--bg2);z-index: 3;">
                <div style="height: 50px;width: 50px;min-width:50px;display: flex;justify-content: center;align-items: center;fill:var(--primary-color)">
                ...
                <div class="project-nav-bar">
                    <div v-for="(path, index) in pathGenerator">
                        <span class="project-path">{{ path.label }}</span>
                        ...
                        <div class="project-cell-row" v-for="member in [...(selectedProject?.manager || []), ...(selectedProject?.members || [])]">
                            ...
                            <div class="project-cell cell-width" data-label="成果目標・昇給課題">
                                <CommandButton 
                                    ...
                                    :buttons="[
                                        { title: '編集', action: () => jumpToGoal(member)},
                                    ]"
                                />
...
import ProjectEdit from './ProjectEdit.vue';
    ...
    const pathGenerator = computed(() => {
        const relatedRoutes: RouteLocationMatched[] = route.matched.filter(
            (rt) => rt.name !== 'projectdetail' && rt.name !== 'project'
        );
