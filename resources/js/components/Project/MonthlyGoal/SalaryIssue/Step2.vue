        ...
        <div>
            ...
            <div class="si-box">
                ...
                <table cellspacing="0" cellpadding="8" style="border-collapse: collapse">
                    <thead>
                        <tr>
                            <th style="padding: 10px;" class="levelTitle">等級</th>
                            <th style="padding: 10px;" class="levelTitle">自己</th>
                            <th style="padding: 10px;" class="levelTitle">組織</th>
                            <th style="padding: 10px;" class="levelTitle">社会</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="grade in grades" style="text-align: center;">
                            <td style="border: solid thin var(--formBorder);padding: 5px 30px;">{{ grade.level }}</td>
                            <td style="border: solid thin var(--formBorder);padding: 5px 30px;">{{ grade.self }}</td>
                            <td style="border: solid thin var(--formBorder);padding: 5px 30px;">{{ grade.organization }}</td>
                            <td style="border: solid thin var(--formBorder);padding: 5px 30px;">{{ grade.society }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        <tr v-for="theme in themes" :key="theme">
                            ...
                            <td @click="setTheme(level, theme)" :class="{'!cursor-not-allowed opacity-70 hover:bg-[var(--background-color)]' : !possibleThemes.includes(filteredIssues(level, theme)[0].title)}" class="selectable-theme align-top" v-for="level in levels" :key="level">
                                <div class="p-[10px]">
                                    <div class="flex items-center gap-2">
                                        <div v-if="possibleThemes.includes(filteredIssues(level, theme)[0].title)">
                                            <div class="h-[18px] min-w-[18px] w-[18px] flex rounded-full bg-[#64bc44]"> 
                                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="10" class="ml-[4px]" fill="#fff" viewBox="0 0 38 32">
                                        ...
                                        <div>
                                            <strong>{{ filteredIssues(level, theme)[0].title }}</strong>
                                        </div>
                                    </div>
                                    
                                    <p class="issue-content">{{ filteredIssues(level, theme)[0].content }}</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
