<template>
    <div class="flex flex-col gap-[30px] relative" v-if="issue">
        ...
        <MessageArea which="salary_issue" :item="issue" :key="`message-area-goal-${goal.id}`" @refresh="() => refresh()"/>
...
const { salaryIssueStatus, salaryIssueStatuses, evaluationData } = useGoal()
