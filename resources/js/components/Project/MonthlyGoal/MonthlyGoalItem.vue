<template>
    <div class="goal-detail relative gap-2">                
        ...
        <ItemStatus 
            :style="{
                color: goalBadge.length ? 'tomato' : 'var(--primary-color)'
            }" 
            :type="'project_goal'" 
            :status="item?.status" 
        ...
        >
            <template #badge>
                <span class="side-notification" style="position: unset;width:15px;z-index: 1;" v-if="goalBadge.length">
                    {{ goalBadge.length }}
                </span>
            </template>
        </ItemStatus>
        <!-- <div 
            class="kadai-content bg-[var(--bg3)] text-[12px] px-2 w-fit" 
            :style="{
                color: goalBadge.length ? 'tomato' : 'var(--primary-color)'
            }"
        >
            <div @click="showStatusLog = !showStatusLog" class="flex items-center gap-1 cursor-pointer">
                <p>{{ goalStatus(item?.status) }}</p>
                <span 
                    class="side-notification" 
                    style="position: unset;width:15px;z-index: 1;" 
                    v-if="goalBadge.length"
                >
                    {{ goalBadge.length }}
                </span>
                <div v-if="item.status_logs && item.status_logs.length">
                    <UserPanel disable-instant size="15" :user="item.status_logs[item.status_logs.length - 1]?.user"/>
                </div>
            </div>
            <div v-if="showStatusLog">
                <p>ステータス更新履歴</p>
                <div v-if="item.status_logs && item.status_logs.length" class="flex flex-col gap-2 py-2">                
                    <div v-for="log in item.status_logs" class="">
        ...
        <div class="flex items-center">
            ...
            <div class="flex items-center ml-3">
                ...
                <div v-if="commentBadge.length" class="side-notification side-notification--comment-only ml-1" style="position: unset;z-index: 1;">
                    {{ commentBadge.length }}
                </div>   
            </div>    
            <div class="ml-auto">
        ...
        <div v-if="item?.salary_issue" class="mt-[5px]">
            ...
            <!-- <div>
                <div class="kadai-content flex items-center bg-[var(--bg3)] text-[12px] px-1 w-fit" :style="{color: badge.salaryIssueByFilter([{by: 'goal_id', value: item.id},]).length ? 'tomato' : 'var(--primary-color)'}">{{ salaryIssueStatus(item?.salary_issue?.status) }}
                    <span class="side-notification" style="position: unset;width:15px;z-index: 1;" v-if="badge.salaryIssueByFilter([{by: 'goal_id', value: item.id},]).length">{{ badge.salaryIssueByFilter([{by: 'goal_id', value: item.id},]).length }}</span>
                </div>
            </div> -->
            <ItemStatus 
                :style="{color: badge.salaryIssueByFilter([{by: 'goal_id', value: item.id},]).length ? 'tomato' : 'var(--primary-color)'}"
                :type="'salary_issue'" 
                :status="item?.salary_issue?.status" 
                :logs="item.salary_issue.status_logs"
            >
                <template #badge>
                    <span class="side-notification" style="position: unset;width:15px;z-index: 1;" v-if="goalBadge.length">
                        {{ goalBadge.length }}
...
const goalBadge = computed(() => {
    if(props.projectId){
        return badge.goalsBadgeByFilter([{by: 'id', value: props.item.id}, {by: 'project_id', value: Number(props.projectId)}])
    }
    return badge.goalsBadgeByFilter([{by: 'id', value: props.item.id}])
})
...
const commentBadge = computed(() => {
    return badge.goalIssueCommentBadgeByFilter([{by: 'project_goal_id', value: props.item.id}])
})
