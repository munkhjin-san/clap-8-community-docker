<template>
    <div class="kadai-content bg-[var(--bg3)] text-[12px] px-2 w-fit text-[var(--primary-color)]">
        <div @click="showStatusLog = !showStatusLog" class="flex items-center gap-1 cursor-pointer flex-wrap">
            <p :class="{'!text-[tomato]' : attentionGoal}">{{ goalStore.goalStatus(status) }}</p>
            ...
            <div v-if="logs && logs.length" class="flex flex-col gap-2 py-2 text-[var(--primary-color)]">                
                <div v-for="log in logs" class="">
                    <div class="flex flex-wrap text-[11px]">
                        ...
                        <div v-if="log.before_number && log.after_number">{{ goalStore.goalStatus(log.before_number) }} → {{ goalStore.goalStatus(log.after_number) }}</div>
                    </div>   
                </div>
            </div>
...
const goalStore = useDashboardGoalsStore()
...
const attentionGoal = computed(() => {
    ...
    if(auth.isBoss){
        ...
        && goalStore.managersGoals.some(user => user.outcome_goals.some(goal => goal.id === props.item.id))
    }
    if(auth.isAdmin) {
