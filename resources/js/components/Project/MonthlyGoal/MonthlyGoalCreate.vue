<template>
    <Modal @close="emit('close')" :loader="firstLoad">
        <template #title>
        ...
        <template #content>
            ...
            <div class="si-box">
                ...
                <select class="dropDownSelector taskDateTimePicker w-fit" id="selectProject" style="max-width: 100%;" v-model="targetProject">
...
const selectedDate = computed(() => {
    const [year, which_half] = props.activeSpan.split('-')
    return {
        year: Number(year),
...
const getEvaluationData = async() => {
    ...
    usersProjects.value = response && response.projects ? response.projects : []
    if(usersProjects.value.length > 0 && !targetProject.value){
        targetProject.value = usersProjects.value[0]
    }
...
const saveOutcomeGoal = async(status: number) => {
    ...
    const span = props.activeSpan as string
