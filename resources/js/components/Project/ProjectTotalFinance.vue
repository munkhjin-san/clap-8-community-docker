                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th v-if="hasPrivilage" :rowspan="2" class="sticky-right comment-cell">コメント</th>
                                    </tr>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <template v-for="(data, projectName) in dataByMonth" :key="projectName">
                                        <tr>
                                            <td class="p-name sticky-left first-col" :rowspan="3">{{ projectName }}</td>
                                            ...
                                            <template v-for="p in periods" :key="p.period">
                                                ...
                                                <td>
                                                    <div class="inner-col"><span class="mobile">売上</span>{{
                                                        amountOfMoneyParser(data?.[p.month]?.yearly_plan.sales) }}</div>
                                                </td>
                                                <td>
                                                    <div class="inner-col"><span class="mobile">販管費</span>{{
                                                        amountOfMoneyParser(data?.[p.month]?.yearly_plan.expense) }}</div>
                                                </td>
                                                <td>
                                                    <div class="inner-col"><span class="mobile">利益</span>{{
                                                        amountOfMoneyParser(data?.[p.month]?.yearly_plan.sales -
                                                        data?.[p.month]?.yearly_plan.expense) }}</div>
                                                </td>
                                                <td>
                                                    <div class="inner-col"><span class="mobile">利益率</span>{{
                                                        percentizer(data?.[p.month]?.yearly_plan).display }}</div>
                                                </td>
                                            </template>
                                            
                                            <td v-if="hasPrivilage" class="sticky-right comment-cell"></td>
                                        </tr>
                                        <tr>
                                            ...
                                            <template v-for="p in periods" :key="p.period">
                                                <td>
                                                    <div class="flex items-center gap-[5px]">
                                                        <div class="inner-col"><span class="mobile">売上</span>{{
                                                            amountOfMoneyParser(data?.[p.month]?.profit.sales) }}</div>
                                                        <DeltaNumbers type="sales" :planned="data?.[p.month]?.yearly_plan.sales"
                                                            :actual="data?.[p.month]?.profit.sales" />
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="flex items-center gap-[5px]">
                                                        <div class="inner-col"><span class="mobile">販管費</span>{{
                                                            amountOfMoneyParser(data?.[p.month]?.profit.expense) }}</div>
                                                        <DeltaNumbers type="expense" :planned="data?.[p.month]?.yearly_plan.expense"
                                                            :actual="data?.[p.month]?.profit.expense" />
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="flex items-center gap-[5px]">
                                                        <div class="inner-col"><span class="mobile">利益</span>{{
                                                            amountOfMoneyParser(data?.[p.month]?.profit.profit) }}
                                                        </div>
                                                        <DeltaNumbers type="profit"
                                                            :planned="data?.[p.month]?.yearly_plan.sales - data?.[p.month]?.yearly_plan.expense"
                                                            :actual="data?.[p.month]?.profit.profit" />
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="flex items-center gap-[5px]">
                                                        <div class="inner-col"><span class="mobile">利益率</span>{{
                                                            percentizer(data?.[p.month]?.profit).display }}</div>
                                                        <DeltaNumbers type="profit_rate"
                                                            :planned="percentizer(data?.[p.month]?.yearly_plan).value"
                                                            :actual="percentizer(data?.[p.month]?.profit).value" />
                                                    </div>
                                                </td>
                                            </template>
                                        ...
                                        <tr>
                                            <td class="sub-name sticky-left second-col flex gap-1 items-center">
                                                <div v-if="showAnyArrow(projectName as string)" class="flex" title="計画との差が大きい月です">
                                                    <svg fill="tomato" style="transform: rotate(180deg);" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 30 30">
                                                        <path d="M14.978 0C6.735-.055-.129 6.931.002 15.153c-.028 8.166 6.815 14.939 14.976 14.811v-.04c.965.012 1.935-.068 2.889-.243 4.817-.861 9.056-4.274 10.937-8.8C32.986 11.04 25.688-.021 14.978 0m0 27.903C6.08 27.659-.075 18.755 3.433 10.373 7.813.292 22.129.294 26.49 10.385c3.512 8.225-2.605 17.404-11.512 17.518m-1.735-13.968c-.293 2.283-.156 4.58-.125 6.873l.166 2.289c.304 2.068 3.234 2.088 3.548 0 .186-1.523.193-3.051.205-4.58.028-1.53.044-3.058-.164-4.582-.334-2.082-3.284-2.104-3.63 0m-.344-4.565c.115.303.278.565.465.811.473.371 1.062.634 1.685.627 1.248.021 2.335-1.09 2.278-2.331-.015-.643-.308-1.218-.729-1.681-1.906-1.558-4.534.238-3.699 2.574"/>
                                                    </svg>
                                                </div>
                                            ...
                                            <template v-for="p in periods" :key="p.period">
                                                ...
                                                <td>
                                                    <div class="flex items-center gap-[5px]">
                                                        <div class="inner-col"><span class="mobile">利益率</span>{{
                                                            percentizer(data?.[p.month]?.settlement).display }}</div>
                                                        <DeltaNumbers type="profit_rate"
                                                            :planned="percentizer(data?.[p.month]?.profit).value"
                                                            :actual="percentizer(data?.[p.month]?.settlement).value" />
                                                    </div>
                                                </td>
                                            </template>
                                            
                                            <td v-if="hasPrivilage" class="sticky-right comment-cell">
                                                <div class="inner-col">
                                                    <span class="mobile">コメント</span>
                                                    <div class="flex items-center gap-2 cursor-pointer">
                                                        <svg fill="var(--primary-color)" height="15" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 33" @click="selectProjectComment(projectName)">
                                                        ...
                                                        <span v-if="commentCount[projectName] > 0" class="text-xs">{{ commentCount[projectName] }}</span>
                                                        <span class="side-notification" style="position: unset;" v-if="financeTotalBadge(projectName)">{{ financeTotalBadge(projectName) }}</span>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
...
const defaultStart = DateTime.fromObject({ year: currentMonth.month >= 3 ? currentMonth.year : currentMonth.year - 1, month: isMobile() ? currentMonth.month : 3, day: 1 })
...
const financeTotalBadge = (name: string) => {
    const findProject = props.projects.find(p => p.name === name)
    return badge.financeCommentBadgeByFilter([{ by: 'project_id', value: findProject?.id}]).length
}
