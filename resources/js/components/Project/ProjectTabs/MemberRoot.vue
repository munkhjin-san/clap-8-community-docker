<template>
    <div class="h-full">
        <div class="sub-tab-container p-5 bg-[var(--background-color)]" v-if="['project-member-list', 'project-member-role', 'project-member-assign'].includes(route.name as string)">
            ...
            <router-link v-if="auth.isBoss || auth.isAdmin || isManager" :to="{name: 'project-member-assign'}" :class="{'selected-sub-tab': route.name === 'project-member-assign'}" class="sub-tab-item no-underline hover:text-inherit hover:no-underline">アサイン<span class="text-[10px] bg-[var(--bg3)] px-1 rounded-full ml-2">BETA</span></router-link>
        </div>
