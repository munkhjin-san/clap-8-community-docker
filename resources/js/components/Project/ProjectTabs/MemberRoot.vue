<template>
    <div class="h-full">
        <div class="sub-tab-container p-5 bg-[var(--background-color)]" v-if="['project-member-list', 'project-member-role', 'project-member-assign'].includes(route.name as string)">
            ...
            <router-link :to="{name: 'project-member-assign'}" :class="{'selected-sub-tab': route.name === 'project-member-assign'}" class="sub-tab-item no-underline hover:text-inherit hover:no-underline">アサイン</router-link>
        </div>
