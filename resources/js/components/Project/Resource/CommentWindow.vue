    ...
    <div class="gantt-overlay" @click="emit('close')">
        <div class="gantt-overlay-inner" ref="createModal" @click.stop>
            <div class="flex items-center h-[60px]">
                ...
                <h4 class="max-w-[calc(100% - 120px)] overflow-hidden overflow-ellipsis whitespace-nowrap">
                  {{ DateTime.fromFormat(period, 'yyyy-MM').toFormat('yyyy年M月') }} {{ memberName }}コメント
            ...
            <div class="c-b">
                ...
                <div class="c-b-i">
                    <div id="typeArea" @keydown.enter="entersend" @input="onInput" ref="commentText" contenteditable="plaintext-only" class="typeBoxArea taskCommentArea"></div>
...
const commentsList = ref<ResourceComment[]>([])
...
const replyComment = ref<ResourceComment | null>(null)
...
const send = async () => {
  ...
  const data = await api.post('/project_resource_comment', payload)
...
const getComments = async () => {
  const data = await api.get('/get_project_resource_comments', { member_name: props.memberName, period: props.period })
...
watch(() => [props.period, props.memberName], async () => {
  loading.value = true
  await getComments()
})
</script>
