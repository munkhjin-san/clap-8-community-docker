    ...
    <div class="flex flex-col" :class="{'items-end' : comment.author.id === auth.activeUser.id }" @click.stop="emit('edit', null)">
        <div class="c-c-text" >
            ...
            <div :class="['c-c-w', {'c-c-active' : editable && editable == comment.id}]">
                ...
                <div class="flex">
                    <div @click.stop="toggleRemind" :title="reminded ? 'リマインドから外す' : 'リマインド'" class="reactButton ml-auto" :class="{ reactOn: reminding }" style="margin: 5px -10px -10px auto">
...
import { ResourceComment } from '@/interface/projectInterface';
    ...
    const toggleRemind = async () => {
        ...
        try {
            await api.post('/project_comment_remind', { id: props.comment.id, kind: 'resource' })
            emit('reload')
        } finally {
