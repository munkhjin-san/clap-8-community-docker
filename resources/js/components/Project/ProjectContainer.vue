                    ...
                    <div class="project-cell project-title-cell">                        
                        <div class="flex w-full">
                            <div class="flex gap-2 items-center relative w-full">
                                <p class="max-w-[calc(100%-60px)] overflow-hidden whitespace-nowrap text-ellipsis"><span v-if="project.status != 'running'">【未締結】</span>{{ project.name }}</p>
...
const outerClass = computed(() => {
  if (state.value === 1) return "is-red";
  if (state.value === 2) return "is-orange";
  return "is-default";
});
...
const cycleColor = () => {
    state.value = ((state.value + 1) % 3) as ColorState;
}
...
const sortedProjects = computed(() => {
  ...
  return searchResults.value.filter(project => {
    ...
    const badgeOk =
      badgeState === 0 ||
      (badgeState === 1 && !!project.has_confirm_badge) ||
      (badgeState === 2 && !!project.has_comment_badge);

    return managerOk && memberOk && statusOk && badgeOk;
  });
});
