<template>
        ...
        <div class="goals-wrap">
            <div style="overflow: hidden; position: relative;height:100%;">
                    ...
                    <div v-if="projectGoals.length" v-for="goal in projectGoals" style="position: relative">                     
                        <div class="goal-detail cursor-pointer" @click="router.push({name: 'goal-more', params: { goalId: goal?.id}})" style="position: relative;gap:10px;margin-bottom: 20px;">
                            ...
                            <div>
                                ...
                                <div v-if="badge.goalIssueCommentBadgeByFilter([{by: 'project_goal_id', value: goal.id}]).length" class="mt-[10px] text-[orange]">未読メッセージ{{ badge.goalIssueCommentBadgeByFilter([{by: 'project_goal_id', value: goal.id}]).length }}件</div>                         
                            </div>
                ...
                <div v-if="memberData && ((auth.id === memberData.id) || isManager)" title="新規作成" id="boardCreate" class="createBoardButton fileNewButton" @click="createOutcomeGoal = true" :style="{zIndex: 7}">
