        ...
        <div id="createModal" class="chatCreate" ref="createModal" @mousedown.stop>
            ...
            <div style="margin-top: 15px;height: -webkit-fill-available;overflow:hidden auto;">                
                <div style="height: -webkit-fill-available;user-select: none;">                    
                    ...
                    <div v-if="filteredPendingMembers.length" style="margin-bottom: 15px;">
                        <div style="font-weight: 600;margin: 15px 0;">{{ $t('pendingMembers') }} ({{ filteredPendingMembers.length }})</div>
                        <div class="suggested-list">
                            <div :key="member.id" v-for="member in filteredPendingMembers">
                                <div class="suggested-wrap">
                                    ...
                                    <div v-if="isRealMember" style="margin-left: auto;">
                                        <div @click="sendAdminResponse(member, 1)" class="commentEditButton" style="margin-left:auto">{{ $t('acceptPartnerRequest') }}</div>
                                        <div @click="sendAdminResponse(member, 0)" class="commentEditButton">{{ $t('denyDenyPartnerRequest') }}</div>
                                    </div>                                    
                                </div>
                            </div>
                        </div>
                    </div>
                    <div v-if="filteredWaitingMembers.length">
                        <div style="font-weight: 600;margin: 15px 0;">{{ $t('waitingMembers') }} ({{ filteredWaitingMembers.length }})</div>
                        <div class="suggested-list">
                            <div :key="member.id" v-for="member in filteredWaitingMembers">
                                <div class="suggested-wrap">
                                    <UserIcon :user="member" imgClass="userNormalIcon" imgStyle="opacity: 0.6;" size="30"/>
                                    <router-link style="opacity: 0.6;" :to="`/profile/${member.id}`" class="suggested-user-name user-link">{{ member.name }}</router-link>
            ...
            <div style="text-align: center;margin-top: auto;" v-if="isRealMember">
                <button @click="$emit('setInvite', board)" style="padding: 10px 15px;" :class="['commentEditButton']">{{ $t('invite') }} </button>
            </div>
    ...
    export default {
        ...
        computed:{
            ...
            admins(){
                ...
                this.board.board_to_users.filter(ob => ob.admin_flag == 1 && ob.member_status == 1).map(ob => ob.user) : []
            },
            filteredAdmins(){
            ...
            members(){
                return this.board && this.board.board_to_users && this.board.board_to_users.length ? 
                this.board.board_to_users.filter(ob => ob.admin_flag == 0 && ob.member_status == 1).map(ob => ob.user) : []
            },
            filteredMembers(){
                const searchText = this.keyword.toLowerCase();
            ...
            pendingMembers () {
                return this.board && this.board.board_to_users && this.board.board_to_users.length ? 
                this.board.board_to_users.filter(ob => ob.admin_flag == 0 && ob.member_status == 0 && ob.invited_by == ob.user_id).map(ob => ob.user) : []
            },
            filteredPendingMembers(){
                const searchText = this.keyword.toLowerCase();
                return this.pendingMembers.filter(user => user.name.toLowerCase().includes(searchText));
            },
            waitingMembers () {
                return this.board && this.board.board_to_users && this.board.board_to_users.length ? 
                this.board.board_to_users.filter(ob => ob.admin_flag == 0 && ob.member_status == 0 && ob.invited_by !== ob.user_id).map(ob => ob.user) : []
            },
            filteredWaitingMembers(){
                const searchText = this.keyword.toLowerCase();
                return this.waitingMembers.filter(user => user.name.toLowerCase().includes(searchText));
            },
            isRealMember(){
                return this.board && this.board.board_to_users && this.board.board_to_users.length && 
                this.board.board_to_users.filter(ob => ob.member_status == 1 && ob.user_id == this.$store.state.user.id).length
            }
