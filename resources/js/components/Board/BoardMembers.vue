    ...
    <div @mousedown="closeModal" class="overlay" style="z-index: 31;font-size:14px">
        <div id="createModal" class="chatCreate" ref="createModal" @mousedown.stop>
            ...
            <div style="text-align: center;margin-top: auto;" v-if="checkAdminAccess">
                <button @click="$emit('setInvite', board)" style="padding: 10px 15px;" :class="['commentEditButton']">メンバー追加</button>
            </div>
        </div>
...
import UserIcon from './Mixed/UserIcon.vue'
    export default {
        props:['board'],
        emits:['close', 'afterRequestHandled', 'reload', 'setInvite'],
        data(){
            return{
                searching: 0,
                keyword: '',
