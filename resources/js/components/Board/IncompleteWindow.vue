<template>
    ...
    <div v-if="(incompletedTasksList.length || unsignedMessages.length) && viewModal && !isEdit && !isJumpToMessage" class="overlay">
            
        <div style="display:flex;flex-direction:column;width:100%;height:100%; margin: 20px 0;overflow: hidden auto;">
            <div class="incompleted-title" v-if="incompletedTasksList.length > 0">{{ $tc('expiredTaskWarning', incompletedTasksList.length, {number: incompletedTasksList.length})}}</div>
            <div @click="viewModal = false" class="modalCloseButton cursor-pointer">
                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 32 32" fill="#fff" style="margin: auto;">
                    ...
                    <masonry-wall :items="incompletedTasksList" :options="{width: 400, padding: 0}">
                    ...
                    <div v-if="unsignedMessages.length" style="padding: 0 10px;">
                        ...
                        <masonry-wall :items="unsignedMessages" :options="{width: 400, padding: 0}">
                            <template v-slot:default="{item}">
...
import { ref, onMounted } from 'vue';
    export default {
        ...
        methods: {
            ...
            editTask(task){
                const url = '/chat/' + task.board_id + '?t='+ task.id + '&task_edit=true'
