            ...
            <div class="file-area-container" @click="previewFile(file, index)">
                <div class="flex-centered">             
                    ...
                    <div style="line-height: 1.5;max-width: calc(100% - 35px);margin-left:5px;">
                        ...
                        <p style="font-size: 10px !important;" v-if="file.sign_flag == 1 && canSign(file) && !file.removed_at" class="shared-file-name">サイン依頼</p>
                        <p v-if="file.multiple_flag == 1 && (canSign(file) || file.user_id == auth.activeUser.id)" style="font-size: 13px !important; position:absolute; top:8px; right:27px; background:black; color:white;margin:0;" class="shared-file-name">確認用</p>                      
                    </div>
                </div>   
                ...
                <div @click.stop="menu.setMenu( { id: file.id, name: 'sharedFileMenu'}), fileMenuLayer = 0" id="messageFileMenuButton" style="position: absolute;right: 2px;top: 6px;" class="boardMenuContainer cursor-pointer">                
                ...
                <div @click.stop id="sharedFileMenu" class="boxMenuComment cursor-pointer" v-if="menu.name == 'sharedFileMenu' && menu.id == file.id" :style="{lineHeight:'normal',top: mIndex == 0 ? 'auto' : '20px',bottom: mIndex == 0 ? '40px' : 'auto', right: '25px', zIndex: 4}">
                    <div style="position: relative;">
                        ...
                        <div v-if="fileMenuLayer == 0" style="right:0;overflow:hidden;box-shadow: rgb(60 64 67 / 30%) 0px 1px 2px 0px, rgb(60 64 67 / 15%) 0px 2px">
                            <ul> 
                                <li @click="downloadFile(file)" class="boxMenuItems cursor-pointer">ダウンロード</li> 
                                ...
                                <li v-if="!file.sign_flag && file.user_id == auth.activeUser.id && file.extension == 'pdf'" @click="signRequest(file)" class="boxMenuItems cursor-pointer">サイン依頼</li>
                            </ul>
                        </div>
