    ...
    <div @mousedown="close" class="overlay">
        <div class="users-list-popup" @mousedown.stop>
            ...
            <div style="position:relative; margin-top:20px;">
                <div v-if="messageUsers.userList && messageUsers.userList.length" class="suggested-list">
                    <div :key="user.id" v-for="user in messageUsers.userList">
                        <div class="suggested-wrap">
                            <UserPanel disable-instant with-name :user="user" imgClass="userNormalIcon" size="30"/>             
                            <GanttButton viewType="status" v-if="user?.pivot && messageUsers.isTask" class="ml-auto" :status="user?.pivot.progress_flag" :loading="false"/>         
                        </div>
                    </div>
                </div>
