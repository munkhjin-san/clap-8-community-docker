<template>
    <div @mousedown="$emit('closeMe')" class="overlay" style="z-index:90">            
        <div id="modalContent04" @mousedown.stop> 
            ...
            <span style="position: initial;padding-bottom:10px;display:block;font-size:12px;">{{ requestType === 'sign' ? $t('signRequestMember') : $t('confirmRequestMember')}}</span>
            <span id="userSelectError" style="position: initial;display:none;padding-bottom:10px;" class="valid-error">{{$t('required')}}</span>
            <div id="checkUserSelecter" style="border: 1px solid #ccc; max-height: 60vh; overflow-y: auto;">
                <div @click="checkmMemberSelectAll" style="padding:5px 15px;display:flex;user-select: none;cursor:pointer;">                                
                    <label class="check-container" style="align-self: center;">
                        <input id="allMemberSelector" v-on:change="selectAllMembers" value="" name="" type="checkbox">
                        <span class="checkmark-mini"></span>
                    </label>  
                    <div class="left-panel-items" style="width: auto;padding:5px 0;margin:0;">                                    
                        <div style="align-self: center;">
                            <p style="line-height: 1.3;font-size: 16px;">{{$t('selectAll')}}</p>                                          
                        </div>                                      
                    </div>
                </div>
                <div v-if="requestType == 'confirm'">    
                    <div :key="user.id" v-for="user in $store.state.mentionAbleUsers" style="padding:0 15px;display:flex;">                                
                        <label class="check-container" style="align-self: center;">
                            <input :id="'memberSelector_' + user.user.id" :value="user.user.id" name="memberCheckBox" type="checkbox">
                            <span class="checkmark-mini"></span>
                        </label>  
                ...
                <div v-else-if="requestType == 'sign'">
                    <div :key="user.id" v-for="user in $store.state.signAbleUsers" style="padding:0 15px;display:flex;">                                
                        <label class="check-container" style="align-self: center;">
                            <input :id="'memberSelector_' + user.user.id" :value="user.user.id" name="memberCheckBox" type="checkbox">
                            <span class="checkmark-mini"></span>
                        </label>  
            ...
            <div @click="checkRequestConfirm()" style="margin: 20px auto auto auto;width:auto;" class="groupEditButton cursor-pointer">
                <span v-if="!processing">{{$t('select')}}</span>
...
import UserIconPreLoad from '../Mixed/UserIcon.vue'
    export default {
        ...
        methods:{
            ...
            checkRequest(id){
                // var all_users = this.board_record.board_to_users.map(obj => obj.user).map(obj => obj.id);
                // var new_user = all_users.filter(value => value != this.my_id);
                const checkboxes = document.querySelectorAll('input[name="memberCheckBox"]:checked');
                ...
                // return
                axios.post('/check_request_api', params).then(             
                    response => {                                   
                        this.$emit('reload')
                        this.$emit('closeMe')
