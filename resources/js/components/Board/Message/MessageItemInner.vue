        ...
        >
            <div :class="['mobileMessageBody', 'mb-2', { 'reached' : urlMessage.id == message.id}, { emojiOnly: (message.emoji_flag == 1 || message.emoji_flag == 2) && !message.message_reply && !message.message_quot, editIsOn:editing, 'mb-35':editing && unreadMessages.id == message.id}]">
                <div class="message-top-block">
                    <div v-if="!compact" class="flex items-center gap-[10px]">
                        <div class="relative">
                    ...
                    <div v-if="!compact" class="messageIconContainer">
                        ...
                        <ItemMenu v-if="!compact && mode !== 'remind' && message.deleted_at == null && !message.draft_flag" type="share" :items="shareMenuItems" fit="boardListInner"/>
                        <ItemMenu v-if="!compact && mode !== 'remind' && message.deleted_at == null" ref="itemMenuRef" :items="messageMenuItems" fit="boardListInner"/>
                    </div>                                           
                </div>    
