<template>
    <div ref="commentScrollEnd" :id="'queueMessage_' + message.u_id" class="messageBoxRoot" :class="{ selfMessage: message.user_id == $store.state.user.id}">
        <div 
                ...
                <div :id="'reply_' + message.id" class="commentHeder" style="position:relative;">
                    ...
                    <div class="column-03" style="position: absolute;top: -40px;right: -17px;display: flex;"> 
                        <div v-if="message.error && !resending" style="font-size: 11px;color: tomato;bottom: -20px;white-space: nowrap;right:0;display:flex;align-items:center">
                            ...
                            <span style="margin-left:5px">{{ $t('failedToSendMessage') }}</span>
                        </div>                      
            ...
            <div :class="message.user_id == this.$store.state.user.id ? 'commentTextBoxRight' : 'commentTextBoxLeft'" style="background:transparent">               
                            ...
                            <div class="file-area-container" >
                                <div class="flex-centered">   
                                    <div v-if="file.mime_type == 'image'" style="max-width:65px;height:40px;display: flex;">
                                        <img style="max-width:100%;margin:auto;max-height:100%;" :src="$store.state.baseLocation + '/temp_upload/'+ file.id + '.' + file.extension">
                                    </div>
            ...
            <div v-if="message.error && !resending" style="color: var(--primary-color);
                ...
                <div @click="resendMessage" style="display:flex;align-items:center;cursor:pointer">
                    ...
                    <span style="margin-left:3px">{{ $t('resend') }}</span>
                </div>
                <div @click="$parent.$parent.$emit('removeError', message.id)" style="display:flex;align-items:center;margin-left:10px;cursor:pointer">
                    ...
                    <span style="margin-left:3px">{{ $t('delete') }}</span>
                </div>
            </div>
...
import FileIcon from '../Mixed/FileIcon.vue';
    export default {
        props: ['message', 'openedBoard', 'qIndex', 'messageListType'],
        data(){
            return{
                resending: false
            }
        ...
        mounted() {  
            setTimeout(()=> {
                ...
                if(container){                    
                    let uKey = this.$store.state.scrollUkey
                    let newUKey = uKey == -1 ? -2 : -1                       
                    this.$store.commit('setScrollUkey', newUKey)
                    container.scrollTop = uKey
                }                
            },0)   
            this.sendMessage();            
        },
        ...
        methods:{
            sendMessage(flag){
                ...
                const params = {
                    ...
                    app_name: 'board',
                    record_id: this.message.record_id,
                    message_id: null,
                    reply_flag: this.message.reply_flag,
                    reply_id: this.message.reply_id,
                    quot_flag: this.message.quot_flag,
                    quot_id: this.message.quot_id,
                    forward_message_id: this.message.forward_message_id,
                    mentioned_users: this.message.mentioned_users,
                    emoji_flag: this.message.emoji_flag, 
                    my_id: this.$store.state.user.id,
                    selected_quot_text: this.message.quot_message,
                ...
                .catch((error) => {
                    if(error){
                        ...
                        this.$parent.$parent.$emit('sendError', this.message)
                ...
                .then(response => {                  
                    ...
                    if(response && response.data && response.data.success && response.data.u_id == this.message.u_id){
                        this.$parent.$parent.$emit('sentMessage', this.message)
                        this.$parent.$parent.$emit('resetReplyQuot')
                        this.resending = false
                        let u_list = this.$store.state.tempUniqueId
                        u_list = u_list.filter( ob => ob !== this.message.u_id)                                
                        this.$store.commit('setTempUniqueId', u_list);
                    }
