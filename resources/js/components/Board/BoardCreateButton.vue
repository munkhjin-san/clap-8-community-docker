<template>
    <div :class="{hiddenButton : createHidden}" :title="$t('createBoard')" id="boardCreate" class="createBoardButton fileNewButton" @click.stop="$parent.$emit('boardCreate', 0)" @touchstart.stop>
        ...
        <div id="newBoardSelector" style="position:realitve;" v-if="$store.state.menu.name == 'newBoardSelector' && $store.state.menu.id == 1000">
            <div id="newBoardSelect" class="boxMenuComment" style="bottom: 40px;right: 20px;top: auto;z-index: 30;text-align: left;width: max-content;">
                <ul>
                    <li @click.stop="$parent.$emit('boardCreate', 0), $store.commit('setMenu', {name: null, id: null})" class="boxMenuItems cursor-pointer">{{ $t('groupBoard') }}</li>
                    <li @click.stop="$parent.$emit('boardCreate', 1), $store.commit('setMenu', {name: null, id: null})" class="boxMenuItems cursor-pointer">{{ $t('privateBoard') }}</li>
