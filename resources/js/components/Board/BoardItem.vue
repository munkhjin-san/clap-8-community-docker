<template>
    <div :id="'board_item_' + item.id" class="left-panel-wrap">
            ...
            <div id="boardBoxMenu" class="boxMenu boardMenuIcon" v-if="menu.name == 'boardBoxMenu' && menu.id == item.id" style="top: 25px;right: 40px;z-index:6;">
                <ul>
                    <li v-if="item.private_flag == 0 && isAdmin" class="boxMenuItems cursor-pointer" @click="edit(item);closeMenu()">編集する</li>
                    <li v-if="item.private_flag == 0 && isAdmin" class="boxMenuItems cursor-pointer" @click="members(item);closeMenu()">メンバー管理</li>
                    <li class="boxMenuItems cursor-pointer" @click="detail(item);closeMenu()">詳細情報</li>                    
                    <li class="boxMenuItems cursor-pointer" @click="pin(item);closeMenu()">{{pinCheckOn ? 'ピン留めを外す' : 'ピン留めする'}}</li>
                    <li v-if="item.private_flag == 0" class="boxMenuItems cursor-pointer" @click="leave(item);closeMenu()">退出する</li>
                    <li v-if="item.private_flag == 1 || item.private_flag == 0 && isAdmin" class="boxMenuItems cursor-pointer" @click="remove(item);closeMenu()">削除する</li>
                </ul>                                            
            </div>
            </Transition>
        ...
        <div class="left-panel-items" @click="open(item)" :class="{leftPanelSelected : isOpened}">
            ...
            <div class="board-c-bar">              
                <div class="boardMenuContainer cursor-pointer" @click.stop="menu.setMenu({name: 'boardBoxMenu', id: item.id})" @touchstart.stop>                                            
    ...
    const lastMessage = computed(() => {
        if(props.item.last_message){
            if(props.item.last_message.message){
                const text = props.item.last_message && props.item.last_message.message ? props.item.last_message.message : ''
