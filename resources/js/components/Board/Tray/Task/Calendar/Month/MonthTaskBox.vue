<template>
    <div class="component-overlay" @click="close">
                ...
                <div style="display:flex; justify-content:space-between;">
                    <div :id="'task_box_' + item.id" class="task-box-inner" :style="{background: taskColor, color: 'var(--primary-color)', position: 'relative', margin: '0 10px', cursor: 'pointer', width:'100%'}">
                            ...
                            <Transition name="modalFade"> 
                                <div id="taskBMenu" class="boxMenuComment cursor-pointer" v-if="$store.state.menu.name == 'taskBMenu' && $store.state.menu.id == item.id" style="z-index:2;top: 20px;right: 15px;box-shadow:none;background-color: unset;">                  
                                    <ul class="messageMenuList">
                                        <li @click="editTask(item), $store.commit('setMenu', {name: '', id: null})" class="boxMenuItems cursor-pointer">{{$t('edit')}}</li>
                                        <li @click="deleteTaskConfirm(), $store.commit('setMenu', {name: '', id: null})" class="boxMenuItems cursor-pointer">{{$t('delete')}}</li>                          
                                    </ul>                                                 
                                </div>
                            </Transition>
                        ...
                        <div v-if="completeButtonFilter(item)" style="display:flex;align-items: center;margin-top: 20px;position:relative;white-space: nowrap;flex-wrap: wrap;gap: 10px 0;">
                            <button class="shift-button" style="margin-right: 7px;" @click="completeTaskBefore(item)" >{{ completeFlag ? $t('inComplete') : $t('finish') }}</button>
                            <button v-if="isToday" @dblclick.stop @click="editTask(item)" class="shift-button" style="margin-right: 7px;">{{$t('edit')}}</button>
...
<script>
    ...
    export default {
        props: ['boxClass', 'completeFlag', 'myColor'],
        mounted(){
        ...
        methods: {
            completeTaskBefore(item){
                emitter.emit('completeTaskBefore', item)
                const data = {
                        active: false,
                        record: null,
                        taskColor: null
            ...
            close(event) {
                ...
                if (event.target.closest('.component-inner')) {
