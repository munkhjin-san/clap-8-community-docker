<template>
    <div class="boardSearchContainer" :class="{bscOpen : searchViewToggle}" style="">        
        <div style="width: calc(100% - 40px);height:40px;display:flex;">
            <div class="boardSearchWrap" :style="searchViewToggle ? 'width:100%' : 'width:0'">                           
                <input 
                    ...
                    class="searchArea" 
                    type="text" 
                    style="font-size:13px;width: -webkit-fill-available;background:#fff;margin: auto 0 auto 5px;padding:3px 5px"
                    autocomplete="off"/>
            </div>
        </div>
        <div @click.stop="searchWindowView" style="min-width:40px;min-height:40px;display:flex;position: absolute;right: 0;top: 0;">
            <svg v-if="!searchViewToggle" style="margin:auto" version="1.1" xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="#fff" viewBox="0 0 32 32">
            ...
            <svg v-else style="margin:auto" version="1.1" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 32 32" fill="#fff">
        ...
        <div id="bsc01" :class="{searchBarView: searchWord.length && searchBoxFocus}" class="search-bar-board">     
            <div @click="openMessageSearch" style="width: 90%; height: 30px;line-height: 28px;font-size: 12px;border-bottom: solid thin #dddddd;margin: auto;display:flex;cursor:pointer;align-items: center;height: 40px;" v-if="searchWord.length">
            ...
            <div v-if="boardSearchResult.length">
                <div @click="$emit('openBoard', board), searchBoxFocus = false" style="padding:5px 10px;cursor:pointer" v-for="board in boardSearchResult">
...
<script>
    export default {
        ...
        methods: {
            ...
            searchWindowView(){
                this.searchViewToggle = !this.searchViewToggle
                emitter.emit('searchStart'); 
                if(this.searchViewToggle){
                    document.getElementById('boardSearchArea').focus();
                }
                if(!this.searchViewToggle){
