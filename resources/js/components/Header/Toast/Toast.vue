<template>
    <div class="cu-toast-mask" @click="clickHandler" @touchstart="clickHandler">
        <div id="cuToastCont" class="cu-toast-container">
            <div class="cu-toast-inner">
                <div v-if="data.closeButton" class="toast-close-button" @click="$emit('close')">
                ...
                <div v-if="data.answers.length"
                    style="display:flex;gap:20px;justify-content: space-evenly;margin: 20px 0 0 0;">
                    <div @click="sendAnswer(answer)" :key="index" v-for="answer, index in data.answers">
                        <div class="cu-answer-button">{{ answer }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    methods: {
        sendAnswer(answer) {
            if (this.data.channel) {
                emitter.emit(this.data.channel, { answer: answer })
            }
        ...
        clickHandler() {
            if(!this.data.touchClose) return
            const cont = document.getElementById('cuToastCont');
        ...
        submitOk() {
            if (event.keyCode == 13) {
                if (this.data.answers.length) {
                    emitter.emit(this.data.channel, { answer: this.data.answers[0] })
                    setTimeout(() => {
                        this.$emit('close')
                    }, 110);
                } else {
