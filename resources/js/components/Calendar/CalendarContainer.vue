<template>
    ...
    <div class="post-root">
        ...
        <!-- <Transition name="modalFade"> -->
            ...
            />
        <!-- </Transition> -->
        <div title="カレンダーへ戻る" class="createBoardButton fileNewButton monthSiwtchButton" @click="switchView">
...
import '../Board/Tray/TrayStyle.scss'
export default{
    props: ['initial_date'],
    data() {
        return {
    ...
    computed:{
        holidays(){
            const holidays = holiday_jp.between(new Date(this.activeYear + '-01-01'), new Date(this.activeYear + '-12-31'));
            return holidays
        },
        daysOfMonth() {
    ...
    methods:{
        ...
        switchView(){
            this.initialLoader = true
            this.viewType == 1 ? this.viewType = 0 : this.viewType = 1
        ...
        setDate(date){
            ...
            this.records = []
            const dateInstance = moment([ date.year, date.month - 1, 1 ]).format('YYYY-MM-DD');
        ...
        getCalendar(day, method){
            ...
            axios.post('/get_calendar_data',{day: day, facilities: fac}).then(response => {  
