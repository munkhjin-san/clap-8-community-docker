<template>
    <div @click.stop="createAtTime" class="w-day-item"  @mouseenter="enter" @mouseleave="leave" style="position: relative;">
        ...
        <Transition name="modalFade">
            <div v-if="dragActive && $store.state.draggingCalendar && $store.state.draggingCalendar.active_user_id == user.id" @mouseup="gotMove(val)" class="month-drop-popup"></div>
        </Transition>
        <MemberRecords
...
import { useStore } from 'vuex';
    ...
    const dropFinish = inject('dropFinish')
        ...
        const enter = () => {
            if(store.state.draggingCalendar){
                if(!moment(props.day.day_full).isSame(moment(store.state.draggingCalendar.date_start), 'date')){
