...
import { useCalendar } from '@/composables/calendar';
    ...
    const weekRecord = useTemplateRef('weekRecord')
    ...
    const fullDay = computed(() => {
        return Math.abs(DateTime.fromISO(props.record.date_start).diff(DateTime.fromISO(props.record.date_end), 'hours').hours) >= 23
    })   
