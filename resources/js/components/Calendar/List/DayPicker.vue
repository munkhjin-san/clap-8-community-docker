    ...
    <div id="cMonthPicker" class="monthPicker">
        ...
        <div id="taskYearPicker" class="month-grid" v-if="$store.state.menu.id == 42 && $store.state.menu.name == 'taskYearPicker'" :style="{right : right ? right : 'auto'}">
            <div class="grid-container-with-day">
                ...
                <div @click.stop="pickerIs = 'year'" class="">{{ this.year }}年</div>
                <div @click.stop="pickerIs = 'month'" class="">{{ this.month }}月</div>
                <div @click.stop="pickerIs = 'day'" class="">{{ this.day }}日</div>
                <div @click.stop="dayNavigation(1)" class="">
            ...
            <div v-if="pickerIs == 'day'" class="day-grid-pick">
                ...
                <!-- <div class="day-picker-wrap" v-for="weeks in dayList">
                    <div v-for="day in weeks">{{ day.day_short }}</div>
                </div> -->
            </div>
        </div>
...
import moment from 'moment'
    export default {        
        props: ['selectedMonth', 'selectedYear','selectedDay', 'right'],
        emits: ['setDate'],
        data(){
            return{
        ...
        watch:{
            pickerIs(after){
                if(after == 'month'){
                    nextTick(() =>{
                        document.getElementById(`m_${this.month}`)
                    })
                }
            },
            selectedMonth: {
        ...
        computed:{
            ...
            dayList(){
                const thisMonth = moment([this.year, this.month - 1]);
            ...
            formatDate(){
                const instance = moment([this.year, this.month - 1, this.day])
                console.log(instance.format('YYYY-MM-DD'))
                return moment([this.year, this.month - 1, this.day]).format('YYYY年M月D日')
            },
        },
        methods:{
            checkSide(date){
                const currentMonth = moment([this.year, this.month - 1])
                const targetMont = moment(date)
                return !currentMonth.isSame(targetMont, 'month')
            },
            dayNavigation(index){
                const instance = moment([this.year, this.month - 1, this.day]).add(index, 'days').format('YYYY-MM-DD')
                console.log(instance)
                this.setDate(instance, true)
            ...
            openMonthPicker(){
                this.pickerIs = 'day';
                if(this.$store.state.menu.name == 'taskYearPicker'){
                    this.$store.commit('setMenu', { name: '', id: null})
                    return
                }
                this.$store.commit('setMenu', { name: 'taskYearPicker', id: 42})
            },
            setYear(y){
                this.year = y
                this.pickerIs = 'month'
            ...
            setDate(d, passive){
                ...
                this.day = parseInt(selected[2])
                this.year = parseInt(selected[0])
                this.month = parseInt(selected[1])
                this.$emit('setDate', {year: this.year, month: this.month, day: this.day, select: true})
                if(!passive){
                    this.$store.commit('setMenu',{ name: '', id: null})
                }
