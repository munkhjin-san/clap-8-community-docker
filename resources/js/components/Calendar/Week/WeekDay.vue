<template>
    <div @click.stop="createAtTime" class="w-day-item">
...
import WeekRecord from './WeekRecord.vue';
export default{
    props: ['day', 'user', 'facilitiesList', 'colors', 'edit', 'delete', 'beforeState'],
    emits: ['addRecord', 'create'],
    components:{
        WeekRecord
    },
    computed:{
        dayRecordsAfter(){
            const user_records = this.day.records.filter(ob =>  ob.calendar_users.map(item => item.id).includes(this.user.id))
            return user_records
        }
    },
    methods:{
