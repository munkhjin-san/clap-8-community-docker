<template>
    ...
    >
        ...
        <div @click.stop="$store.commit('setMenu', {id: record.id, name: 'calendarRecordMenu'})" v-if="expanded && editable" class="boardMenuContainer cursor-pointer" style="align-self: normal;position: absolute;right: 10px;top: 10px;">
        ...
        <div v-if="$store.state.menu.id == record.id && $store.state.menu.name == 'calendarRecordMenu'" id="calendarRecordMenu" class="boxMenu boardMenuIcon" style="top: 25px; right: 35px; z-index: 6;">
            <ul>
                ...
                <li @click.stop="viewDetails = true, $store.commit('setMenu', {id: record.id, name: `cal_${record.id}`})" class="boxMenuItems cursor-pointer">詳細情報</li>
            </ul>
        </div>
...
export default{
    ...
    props: ['record', 'facilitiesList', 'colors', 'viewable', 'editable', 'expanded',],
