<template>
    <div 
        ...
        :id="`cal_${record.id}`" 
        @click.stop="$emit('selectRecord', record, null)"       
    ...
    >
        ...
        <div v-else style="display: flex;">
            ...
            <div style="margin: 1px 0 0 5px;overflow: hidden;" v-if="!expanded && fullDay">{{ truncatedTitle }}</div>
        </div>
        <div v-if="!fullDay || expanded" @mousedown="expanded ? $event.stopPropagation() : false" :class="['cal-card-item', {'wrap cal-selectable' : expanded }]">{{ viewable ? record.title : '予定' }}</div>
        <div v-if="!expanded && !fullDay" class="cal-card-item" style="white-space: nowrap;">{{ time }}</div>
        <div @mousedown.stop v-if="expanded" :class="['cal-card-item', {'wrap cal-selectable' : expanded }]" style="line-height:1.5;margin: 10px 0;display: flex;gap: 10px;align-items: center;">                
        ...
        <div v-if="expanded && viewable">
            <div @mousedown.stop v-if="remarks" class="wrap cal-remark" v-html="remarks"></div>
        ...
        <div @click.stop="$store.commit('setMenu', {id: record.id, name: 'calendarRecordMenu', user_id: $store.state.menu.user_id ? $store.state.menu.user_id : null})" v-if="expanded && editable && record.shift == 0" class="boardMenuContainer cursor-pointer" style="align-self: normal;position: absolute;right: 10px;top: 10px;">
...
<script setup>
    ...
    const truncatedTitle = computed(() => {
        if(props.record.title && props.record.title.length){
    ...
    const pushInstantUser = ((event, id) => {
        if(id == store.state.user.id) return
        const cX = event.clientX;
        const cY = event.clientY;  
        const data = { id: id, cX: cX, cY: cY }
