<template>
        ...
        <div style="display:flex;gap:20px;background:inherit;margin-top:15px;align-items: center;flex-wrap:wrap;">
            <input 
                ...
                placeholder="内容"
                type="text"
                style="padding:0 10px; height:38px;border:1px solid var(--primary-color);color:var(--primary-color); width: 50%;"
                v-model="content"
            />
            ...
            <div v-if="!fileModel" style="display:inline-block; position: relative;">
...
import { useApi } from '@/composables/api';
    ...
    const expenses = defineModel<number | null>('expenses', { default: null })
    const fileModel = defineModel<string | null>('file_path', { default: null })
    ...
    const uploadStart = async(file: File) => {
        if(file){
            ...
            const response = await api.post('/post_grant_upload', formData, {
                loadingRef: loading,
            })
...
<style scoped>
    ...
    .money-input-wrapper {
        ...
        .money-input {
            width: 100%;
            height: 38px;
            padding: 0 28px 0 10px; /* room for suffix */
        ...
        .money-input::-webkit-inner-spin-button {
            -webkit-appearance: none; /* kill spinner in Chrome */
            margin: 0;
        }
