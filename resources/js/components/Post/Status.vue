    ...
    <div class="overlay" @mousedown="emit('close', null)">          
        <div class="chatCreate" style="height:auto;max-width: 70%;" @mousedown.stop>            
            <div class="recordFormTitle" style="display:flex">
                <p>{{ progressReportMode ? '進捗報告' : 'ステータス変更' }}</p>
            ...
            <div class="si-box" v-if="selected > 0 || progressReportMode">
                <LongInput
                    :initialValue="resultMessage"   
                    ref="recordBody"
                    :placeHolder="progressReportMode ? `現在の進捗を入力` : selected == 5 ? `現在の進捗を入力` : `結果内容を入力`"
                    name="recordBody"
                    :rules="progressReportMode ? 'required|max:2000' : 'max:2000'"
                    label="タイトル"
