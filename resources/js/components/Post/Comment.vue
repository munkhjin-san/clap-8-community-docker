    ...
    <div :key="comment.id" class="commentRoot" >
        <div :class="['commentInner']" :style="{position:'relative',padding:'15px',borderRadius: '0', border: editing ? 'solid 2px var(--hoverBorder)' : 'solid 2px transparent', boxSizing: 'border-box', float: comment.user_id == auth.id ? 'right' : 'left'}">
            <div class="commentHeder" style="position:relative;">                                            
                ...
                <div v-if="comment.user_id == auth.id && comment.deleted_flag == 0 && !editing" class="boardMenuContainer cursor-pointer" @click.stop="menu.setMenu( {id: comment.id, name: 'commentBoxMenu'})">
                ...
                <Transition name="modalFade">
                    <div id="commentBoxMenu" class="boxMenu boardMenuIcon" v-if="menu.name == 'commentBoxMenu' && menu.id == comment.id" style="top: 25px;right: 25px;z-index:6;">
                        <ul>
                            <li class="boxMenuItems cursor-pointer" @click.stop="editing = true, closeMenu()">コメントを編集する</li>
