    ...
    <div :class="className" style="">
        <div style="width:100%;display:flex;">
            <div class="searchBarInner" :style="{width: responsive.mobile || className == 'newChatMemberSearch' ? '100%' : '25%',marginLeft: 'auto'}">   
                <input name="postSearchBar" @input="setKeyWord" v-model="keyword" @focus="emit('focus')" @blur="emit('blur')" class="searchBarArea searchInputArea memberSearch" :placeholder="customPlaceHolder ? customPlaceHolder : 'メンバー検索'" type="search" style="margin: 0;width:100%;"/>
                ...
                <div @click="cancelSearch" v-if="keyword.length && searching == 0" class="searchBarCancelButton" style="right: 0;">
                ...
                <div v-if="searching > 0" class="searchBarCancelButton" style="right: 0;z-index:1">
...
<script setup>
    ...
    const props = defineProps(['searching', 'className', 'customPlaceHolder', 'initialValue'])
    const responsive = useResponsive()
    const emit = defineEmits('searchStart', 'setKeyord', 'blur', 'focus')
    const keyword = ref('')
    const timeout = ref(0)
    onMounted(() => {
        if(props.initialValue){
            keyword.value = props.initialValue
        }
    })
    const cancelSearch = () => {
        keyword.value = ''
    ...
    const setKeyord = () => {
        emit('setKeyord', event.target.value)
    }
    const setKeyWord = () => {   
