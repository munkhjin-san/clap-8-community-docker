<template>
        <div style="display:flex;margin: 5px 0;place-content: center;margin-top: auto;">  
            ...
            <button class="search-navi-button" style="display:flex;" @click="setNavi(-1)">
                <svg version="1.1" height="8" viewBox="0 0 20 32" xmlns="http://www.w3.org/2000/svg" style="margin:auto">
                ...
                <div id="pageNavigator01" v-if="possiblePage > 6" class="search-navi-button cursor-pointer" style="position:relative;padding:0;">
                    <p @click.stop="$store.commit('setMenu', {name: 'pageJumperList01', id: 367})" style="padding:10px;">...</p>
                    <Transition name="modalFade">
                    <div v-if="$store.state.menu.name == 'pageJumperList01' && $store.state.menu.id == 367" id="pageJumperList01" class="boxMenu" style="top: auto;bottom:30px;left: 50%; transform: translate(-50%, 0); margin-left: auto; margin-right: auto;max-height: 145px;max-width:145px;overflow: auto;">
                        <p @click="$emit('setActivePage', number), $store.commit('setMenu', {name: '', id: null})" class="boxMenuItems" :key="number" v-for="number in possiblePage">{{number}}</p>
                    </div>
                    </Transition>
...
<script>
    export default {
        ...
        methods:{
            setNavi(index){
                if((this.possiblePage == this.activePath && index == 1) || (this.activePath == 1 && index == -1))return
                this.$emit('setNavi', index)
            }
        } 
    }
</script>
