        ...
        <div class="side-menu-inner" v-if="auth.user">    
            ...
            <router-link :to="{name: 'project'}" v-if="!auth.isPartner && auth.user" :class="['side-menu-route', { selectedRoute: isActive}]">
                <div class="side-menu-route-inner">   
                    ...
                    <span v-if="sideMenuView.active">プロジェクト</span>
                    <span v-if="badge.project.total_sum" class="side-notification">{{ badge.project.total_sum }}</span>      
                </div>                
            </router-link>
            ...
            <router-link :to="{name: 'timesheet'}" v-if="auth.user && (auth.id == 685 || auth.id == 686) || !auth.isPartner" :class="['side-menu-route', { selectedRoute: selectedRoute == 'timesheet'}]">
                <div class="side-menu-route-inner">   
                    ...
                    <span v-if="sideMenuView.active">タイムシート</span>
            ...
            <router-link :to="{name: 'learning'}" v-if="!auth.isRegistered && !auth.isPartner || [825,826,827,828,799,800,829,686].includes(auth.id)" :class="['side-menu-route', { selectedRoute: selectedRoute == 'learning'}]">
                <div class="side-menu-route-inner"> 
                    <svg class="side-app-icon" version="1.1" xmlns="http://www.w3.org/2000/svg" width="31" height="32" viewBox="0 0 31 32" style="width: 20px !important; height: auto;min-width:20px;">
                        ...
                        <path d="M11.194 9.883c0.531 0.531 1.176 0.95 1.866 1.232 0.701 0.283 1.493 0.396 2.194 0.396s1.481-0.192 2.171-0.464c0.69-0.271 1.334-0.678 1.877-1.199 0.531-0.52 0.972-1.153 1.266-1.843s0.452-1.447 0.464-2.205c0-0.758-0.136-1.515-0.418-2.216s-0.712-1.346-1.244-1.888c-0.531-0.543-1.176-0.972-1.888-1.266-0.701-0.271-1.459-0.43-2.216-0.43s-1.527 0.147-2.228 0.441c-0.701 0.294-1.346 0.724-1.877 1.266s-0.961 1.187-1.244 1.888c-0.283 0.701-0.43 1.459-0.43 2.216 0.011 0.758 0.158 1.515 0.452 2.205 0.283 0.701 0.724 1.334 1.255 1.866zM12.698 3.257c0.678-0.667 1.617-1.052 2.567-1.052s1.888 0.373 2.567 1.052c0.339 0.339 0.611 0.735 0.803 1.165s0.294 0.905 0.294 1.391c0 0.486-0.079 0.961-0.26 1.402-0.17 0.452-0.441 0.859-0.78 1.21-0.667 0.712-1.809 1.040-2.623 1.040s-1.922-0.373-2.601-1.063c-0.678-0.69-1.063-1.64-1.052-2.601 0-0.95 0.396-1.888 1.085-2.544z"></path>
                    </svg>
...
import WeatherIcon from '@/components/Global/WeatherIcon.vue';
    ...
    const isActive = computed(() => {
        return route.matched.some(val => val.name === 'project')
    })
