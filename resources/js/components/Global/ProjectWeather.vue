            ...
            <div class="flex flex-col gap-5">
                <div v-for="project in projects" :key="project.id" class="project-card">
                    ...
                    <div class="condition-options">
                        <div :class="['condition-wrap', { selectedBackground: isSelected(num, project.id) }]"
                            v-for="num in [0, 1, 2, 3, 4, 5]" :key="`${project.id}-${num}`"
                            @click="toggleSelection(num, project.id)">
                            <WeatherIcon :class="['condition-icon', { selected: isSelected(num, project.id) }]"
                                :which="num" size="20" />
                        </div>
                    </div>
                </div>
...
const fetchManagingProjects = async () => {
    const list = await api.get("/get_managing_projects");
    projects.value = list;
};
...
const toggleSelection = async (val: number, projectId: number) => {
    ...
    if (projects.value.length === selected.value.length) {  
        await api.post('/update_project_conditions', { selected: selected.value }, {
            toast: '保存しました。'
        })
        fetchManagingProjects()
    }
