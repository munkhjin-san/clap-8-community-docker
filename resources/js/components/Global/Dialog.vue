            ...
            <div ref="cuToastCont" class="cu-toast-container">
                <div class="cu-toast-inner">
                    ...
                    <div style="display:flex;gap:20px;justify-content: space-evenly;margin: 20px 0 0 0;">
                        ...
                        <div
                            @click="sendAnswer(answer.value, index)"  
                            :key="index" 
...
import { computed, ref } from 'vue';
    ...
    const props = defineProps(['confirm', 'notify', 'info', 'options'])
    const cuToastCont = ref(null)
    const selected = ref(null)
    const emit = defineEmits('close', 'handle')
    const answers = computed(() => {
        if(props.options && props.options.answers){
            const answers = []
            props.options.answers.forEach(answer => {
                if(answer.label !== null && answer.value !== null){
                    answers.push(answer)
                }                
            });
            return answers
            ...
            const answers = [{label: 'OK', value: true}]
            if(props.confirm){
                answers.push({label: 'キャンセル', value: false})
            }
            return answers
        }
        
    })
    const sendAnswer = (answer, index) => {
