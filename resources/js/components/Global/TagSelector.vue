...
export default {
    props: ['placeHolder', 'initialValue', 'specialTags'],
    emits: ['updated'],
    created(){
        this.superFetch()
    },
    data(){
        return{
            selectedTag: this.initialValue ? this.initialValue : [],
            tagOptions: [],
            Deselect: markRaw({
    ...
    methods:{
        ...
        //     if(search.length) {
        //         if(this.before && this.before == search) return
        //         this.before = search
        //         loading(true);
        ...
        //     axios.post('/post_get_tags', {key: search, super: false})
        //     .then(response => {
        ...
        superFetch(){
            this.$store.commit('setActiveInput', 'tagSelector')
            console.log('super')
            if(this.superCounter > 0) return
            axios.post('/post_get_tags', {key: '', super: true, special: this.specialTags && this.specialTags.length ? this.specialTags : []})
            .then(response => {
...
<style>
    /* .tagSelectorIndepend  .vs__search{
        padding-left: 13px;
    } */
    .tagSelectorIndepend  .vs__dropdown-menu > ul > li{
        font-size: 14px;
    }
</style>