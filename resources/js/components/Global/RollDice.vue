      ...
      <div class="chatCreate" style="width: unset; height: unset;">
        ...
        <div class="dice-wrapper">
          ...
          <div style="margin-top: 20px;" v-if="!missed && hit !== undefined && hit >= 0 && hit < 800">
            <CommandButton customStyle="height: 30px;":buttons="[{title: hit && hit > 0 ? 'ダブルアップ' : 'サイコロを振る', action:() => hit && hit > 0 ? doubleUp() : startRolling()}]"/>
          </div>
          ...
          <div v-if="hit === 800" style="display: flex; margin-top: 20px; flex-direction: column; gap: 20px; align-items: center; font-size: 13px;">
            <div>{{`最高額おめでとう！<br> 賞金は翌月のリフレッシュ<br>補助金に増額されます。`}}</div>
            <div @click="emit('close', false)" class="nine-answer-button">OK</div>
          </div>
          <div v-if="missed" style="display: flex; margin-top: 20px; flex-direction: column; gap: 20px; align-items: center; font-size: 13px;">
            <p>ハズレ　次回もお楽しみに！</p>
            <div @click="emit('close', false)" class="nine-answer-button">OK</div>
          </div>
        </div>
...
import axios from 'axios';
  ...
  const updateTryFlag = async() => {
      try {
          await axios.put('/task_update_flag', {task_id: props.taskId})
      } catch (e) {
  ...
  const savePrize = async() => {
    ...
    await axios.put('/task_update_prize', params)
    // info('保存しました。')
