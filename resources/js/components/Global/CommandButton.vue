<template>
    <div class="c-button" :style="customStyle" v-for="button in buttons">
        <div @click="emit('select', button)" class="primary-selection" :class="customClass">{{ button.name }}</div>
        <!-- <div v-if="buttons.length > 1" class="divider"></div> -->
        <!-- <div v-if="buttons.length > 1" class="rt-cont" @click.stop="menu.setMenu({id: menu.id == uniqueId ? null : uniqueId, name:'c-popup'})" >
            <svg class="rt-icon" fill="var(--background-color)" :style="{transform: `rotate(${menu.id == uniqueId && menu.name == 'c-popup' ? 90 : 270}deg)`}" version="1.1" width="10" height="10" viewBox="0 0 20 32" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- <Transition name="d-transtion">
            <div v-if="menu.id == uniqueId && menu.name == 'c-popup'" id="c-popup" class="d-popup">
                <div @click="select(button)" class="d-item" v-for="(button, index) in buttons.slice(1)">{{ button.name }}</div>
            </div>
        </Transition> -->
    </div>
</template>
...
import { useMenuStore } from "@/store/menu";
    ...
    const props = defineProps(['buttons', 'customStyle', 'customClass'])
