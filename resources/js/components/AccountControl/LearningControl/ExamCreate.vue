<template>
    <div class="overlay" @mousedown="close(false)">                         
        <div class="chatCreate scrollable" @mousedown.stop>
            <div class="recordFormTitle exam-create__header">
                <p>{{ examData ? '試験を編集する' : '新しい試験を作成する' }}</p>
                <div class="m-close-button exam-create__close" @click="close(false)">
            ...
            <div class="si-box exam-create__settings-grid">
                <div>
            ...
            <div class="si-box">
                <div class="exam-create__question-toolbar">
                    <div class="exam-create__section-title">設問一覧</div>
            ...
            <div class="si-box">
                <span v-if="formError" class="form-error exam-create__error">{{ formError }}</span>
...
import type { LearningExam, LearningExamOption, LearningExamQuestion } from '@/types/learning';
import ExamQuestionEditor from './Authoring/ExamQuestionEditor.vue';
import type { EditableExamOption, EditableExamQuestion } from './Authoring/examEditorTypes';
...
const normalizeOptions = (options?: LearningExamOption[]): EditableExamOption[] => {
    if(options?.length){
...
const normalizeQuestions = (): EditableExamQuestion[] => {
...
const save = async() => {
    ...
    try{
        await learningApi.saveAdminExam(payload, Boolean(props.examData))
        close(true)
    }catch(error){
        console.error(error)
        toast('保存に失敗しました。')
...
.exam-create__settings-grid{
    display: grid;
    gap: 15px;
    grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));
}

.exam-create__question-toolbar{
    align-items: center;
