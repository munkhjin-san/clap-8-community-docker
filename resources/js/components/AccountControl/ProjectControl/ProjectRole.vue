<template>
    <div class="overlay">
        <div class="chatCreate scrollable">
            ...
            <div>
                <div class="si-box">
                    <div style="border: 1px solid var(--primary-color);" class="form-wrapper" :class="{focused: checkedCriteria || focus}">
                        <label style="z-index:5" class="form-plc">
                            職務評価基準
                        </label>                 
                        <drop-selector        
                            style="background-image: unset; margin:0px;border: none;width: 100%;" 
                        ...
                        <template v-slot:no-options="{ search, searching }">
                            ...
                            <span style="font-size: 13px;opacity: 0.5" v-else>職務評価基準を検索するにはキーワードを入力してください</span>
                        </template>                      
                        </drop-selector>
                    </div>
                </div>
                <div class="si-box">
                    <loader-button @triggered="saveRole" content="保存"/>
                </div>
            </div>
        </div>
    </div>
...
const props = defineProps(['editRole'])
const criteriaMaster = ref([])
const checkedCriteria = ref(props.editRole?.role ?? '')
...
const saveRole = async() => {
    const params = {
        id: props.editRole?.id ?? null,
        role: checkedCriteria.value,
    }
    try {
        await axios.put('/save_member_role', params)
        emit('close')
    } catch (e) {
