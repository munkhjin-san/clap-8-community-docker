            ...
            <table>
                ...
                <tbody>
                    <tr v-for="user in searchResults">
                        ...
                        <td>
                            <svg v-if="user?.evaluation?.candidate?.some((c: Candidate) => c.next_candidate == '昇格（職階）')" fill="var(--primary-color)" version="1.1" xmlns="http://www.w3.org/2000/svg" height="10" viewBox="0 0 38 32">
...
import { Candidate, Evaluation, ProjectGoal } from '@/interface/projectInterface';
...
const searchResults = computed(() => {
    if(props.keywords){
        ...
        const deepSearch: (obj: UserWithGoals) => boolean = (obj: UserWithGoals) => {
        ...
        return props.userList.filter((project: UserWithGoals) => deepSearch(project))
    }
    return props.userList 
})
const addEvaluation = (user: User) => {
...
const safeGoals = (user: UserWithGoals) => {
...
const generateEvaluationCsv = () => {
    ...
    const data: any[] = []
    props.userList.forEach((user: UserWithGoals) => {
        data.push({
            "メンバーID": user.id,
            "メンバー": user.name,
...
const positions = computed(() => {
    const pos = props.userList.map((user: User) => user?.positions).filter((pos: { id: number, name: string } | null): pos is { id: number, name: string } => !!pos)
    //make unique
    const uniquePositions = Array.from(new Set(pos.map((p: { id: number, name: string }) => p.id))).map(id => {
        return pos.find((p: { id: number, name: string }) => p.id === id)
    }).sort((a, b) => a.id - b.id)
...
const findTargetUsers = async() => {
    ...
    let d: any[] = []
    const users = props.userList.filter((u: User) => u?.position_id !== undefined && selectedUserPositions.value.includes(u.position_id))
    
    users.forEach((user: UserWithGoals) => {
