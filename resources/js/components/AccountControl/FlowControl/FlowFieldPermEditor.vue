    ...
    <div class="flex flex-col gap-[12px] max-w-[900px]">
        ...
        <div v-for="f in fields" :key="f.id" class="flow-card">
            ...
            <div v-for="(row, i) in rowsFor(f.id!)" :key="i" class="prow">
                ...
                <span v-for="p in PERMS" :key="p.k" class="flow-perm-flag" @click="(row as any)[p.k] = !(row as any)[p.k]">
                    <span class="flow-cbox" :class="{ on: (row as any)[p.k] }">
            ...
            <div class="add-panel">
                ...
                <div class="flags-row mt-[8px]">
                    <div class="flow-perm-flags">
                        <span v-for="p in PERMS" :key="p.k" class="flow-perm-flag" @click="(flagsFor(f.id!) as any)[p.k] = !(flagsFor(f.id!) as any)[p.k]">
                            <span class="flow-cbox" :class="{ on: (flagsFor(f.id!) as any)[p.k] }">
