<template>
    <div style="position: fixed;top: 0;display:flex;height:50px;width: -webkit-fill-available;margin: 0px 22px 0 20px;">
        <div style="width:100%;height:50px;display:flex;min-height: 40px;">
            <div class="searchBarInner" style="width:20%; margin-left:auto"> 
                <!-- :style="{width: $store.state.mobile ? '100%' : '15%',marginLeft: 'auto'}"  -->
                <input @keyup="setKeyord" class="searchBarArea searchInputArea" placeholder="検索" type="text" style="margin: auto 0 auto auto;width:100%;background:#fff"/>
                <div style="position: absolute;left: 10px;top: 1px;display: flex;height: 40px;">
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 32 32" style="margin: 17px auto auto auto;fill:#767676">
...
<script>
    export default {
        methods: {
            setKeyord(){
                this.$emit('setKeyord', event.target.value)
            }
        }
    }
</script>
