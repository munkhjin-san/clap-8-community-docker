    ...
    <div style="background: inherit;">
        <div v-if="$route.name == 'more'" style="background: inherit;">
            <!-- <div class="lessons-topic">
                <div>
                    <div v-if="material" v-html="filteredContent"></div>
                </div>
            </div>
            <div v-if="sectionStatus !== 2" class="post-separetor" style="margin-top: 30px;"></div>
...
<a href=/app/public/user?id=${store.state.user.id}>@${store.state.user.name}</a>
...
`,
                record_id: chat.data,
                mentioned_users: [supportAccountId.value, store.state.user.id],
    ...
    const fail = async (status) => {
        ...
        emitter.emit('setToast', {
            active: true,  
            type: 'info', 
            content: `サポート用の<a href="/board/${chatId}" target="_blank">ボード</a>が作成されました。<br>研修サポート担当からからの連絡をお待ちください。` ,
            closeButton: false, 
            autoClose: false,
            touchClose: false,
            answers: ['OK'],
