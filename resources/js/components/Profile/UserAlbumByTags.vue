                ...
                <div :key="item.id" v-for="item in tagAlbums" class="" style="padding: 15px;background: var(--background-color);">
                    <div class="recordBox-inner">        
                        ...
                        <div class="recordContents" style="line-heigth:1.5;margin-top:0;font-size:14px;line-height: 1.8;margin-top:10px;">
                            <div class="recordContents-inner cursor-pointer" v-for="(album, index) in item.user_album" :key="album.id" @click="previewImage(album, index)">
                                <p>{{ album.title }}</p>
                                <img class="gn-image" style="height:100%;object-fit: cover;" v-if="album.mime_type == 'image'" :src="movSrc(album)" loading="lazy"/>
                                <video class="gn-image" v-else-if="isMov(album.mime_type)" controls="controls" style="pointer-events: none;max-height: 290px;">
...
<script>
    export default {
        props: ['tagText', 'tagAlbums', 'targetId'],
        methods: {
            isMov(type){
            ...
            previewImage(file, index){
                ...
                const old_mov = this.movSrc(file)
