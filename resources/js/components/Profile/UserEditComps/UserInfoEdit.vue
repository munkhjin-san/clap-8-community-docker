...
import { useApi } from '@/composables/api';
    ...
    const userMail = ref(props.UserAllData.work_email)
    const userIntro = ref(props.UserAllData.intro)
    const userMotto = ref(props.UserAllData.motto)
    const userEnjoy = ref(props.UserAllData.enjoy)
    const userRecommend = ref(props.UserAllData.recommend)
    const userPhone = ref(props.UserAllData.phone_number)
    const api = useApi()
    ...
    const profileEditSend = async () => {
        ...
        const params = {
            ...
            work_email: userMail.value,
            intro: userIntro.value,
            motto: userMotto.value,
            enjoy: userEnjoy.value,
            recommend: userRecommend.value,
        };

        processing.value = true;

 
        const response = await api.post('/profile_profile_edit_api', params, {
            toast: '保存しました。',
