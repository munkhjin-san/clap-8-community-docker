                        ...
                        <div style="max-width: 280px;overflow: hidden auto; padding: 0 10px;">
                            <div class="swiper-wrapper vertical-wrapper">
                                <div class="swiper-slide" style="background: none;flex-direction: column;margin-bottom: 30px;" v-for="(mov, index) in movExist" :key="index">
                                    ...
                                    <div v-if="targetId == auth_id" @click.stop="menu.setMenu( { id: mov.id, name: 'userAlbumMenu'})" id="userMenuButton" style="position: absolute;right: 2px;top: 6px;background-color: var(--bg3);" class="boardMenuContainer cursor-pointer">                
                                    ...
                                    <div @click.stop id="userAlbumMenu" class="boxMenuComment cursor-pointer" v-if="menu.name == 'userAlbumMenu' && menu.id == mov.id" :style="{lineHeight:'normal',top:'20px',bottom:'auto', right: '25px', zIndex: 4}">
