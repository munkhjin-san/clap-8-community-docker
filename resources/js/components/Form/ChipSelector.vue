    ...
    <div class="advanced-user-selector-options">
        ...
        <Transition name="selector-accordion">
            <div v-if="viewAccordian" class="group-selector-wrap">    
                <div v-if="chips.length" class="selector-inner">
                    <div v-for="chip in chips" @click="selectChip(chip)" :class="['selector-chip', {'active-chip' : group.some(ob => ob.id === chip.id)}]">
                        <div>{{ chip.name }}</div>
                    </div>
                </div>
            </div>
...
const chips = ref([])
const viewAccordian = ref(group.value.length ? true : false)
...
const getPossibleChips = async () => {
    axios.get(`${props.path}`).then(res => {
        chips.value = res.data
    });
}
const selectChip = (chip) => {
    const index = group.value.findIndex(ob => ob.id === chip.id)
    if (index !== -1) {
        group.value.splice(index, 1)
    } else {
        group.value.push(chip)
    }
