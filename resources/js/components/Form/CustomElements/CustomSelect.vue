                    ...
                    <div :key="answer.id" v-for="(answer, index) in elements">     
                        <div class="flex items-center gap-[10px]">
                            <div class="flex items-center">
                                <input placeholder="選択肢" class="custom-o-input" type="text" :id="`c-check-${answer.id}`" ref="options" v-model="answer.value" @keyup.enter="addItem(index, true)"/> 
                            </div>
                            <div class="flex ml-auto">
...
import AddIcon from '../AddIcon.vue';
    ...
    const options = useTemplateRef<HTMLInputElement[]>('options')
    const addItem = (index: number, jump?: boolean) => {
        ...
        setTimeout(() => {
            if(jump && options.value && options.value.length){
                const el = options.value.find( c => c.id == `c-check-${id}`)
                console.log(el)
                el?.focus()
            }
        });
