<template>
    <div id="dashBoardContainer" class="w-full h-full overflow-hidden relative">
        <div class="w-full h-full overflow-y-scroll bg-[var(--bg3)] relative" :class="{'hidescroll' : route.params.type}" ref="sortParent" @scroll="handleScroll">
            <div class="mem-header-section mobile" :style="{'transform': `translateY(${offset}px)`}">        
                <div class="post-header sticky top-0 z-[11] bg-[var(--background-color)]" >
                    ...
                    <div class="flex items-center ml-auto mr-1">
                        <button class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--bg3)]" @click="router.push({name: 'dashboard-support'})">
                        ...
                        <button class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--bg3)]" @click="refreshAll" v-if="!route.params.type">
                            <svg v-if="!initialLoader && !refreshingDashboard" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 406.7002 448.97456">
            ...
            <FloatButton :hide-on="sortParent" :order="3" @action="customize" class="fixed pc z-[13]" v-if="!route.params.type">
                <template #icon>
            ...
            <FloatButton :hide-on="sortParent" title="サポートデスク" :order="2" class="fixed pc" @action="router.push({name: 'dashboard-support'})">
                <template #icon>
