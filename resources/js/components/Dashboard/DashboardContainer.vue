<template>
    <div id="dashBoardContainer" class="w-full h-full overflow-hidden relative">
        <div class="w-full h-full overflow-y-scroll bg-[var(--bg3)] relative" :class="{'hidescroll' : route.params.type}" ref="sortParent">
                ...
                <div class="post-header sticky top-0 z-[11] bg-[var(--background-color)]" >
                    ...
                    <div class="flex items-center ml-auto mr-1">
                        <button class="relative w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--bg3)]" @click="router.push({name: 'dashboard-support'})">
                            ...
                            <Badge style="left: auto;right:-3px;top:-2px" :count="collection.systemUpdates.length"/>
                        </button>
                        <button @click="customize" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--bg3)]">
                            <svg fill="var(--primary-color)" style="height:18px;width:18px" xmlns="http://www.w3.org/2000/svg" width="24.0183" height="23.85664" viewBox="0 0 24.0183 23.85664">
                                <path d="M20.50182,23.85664c-.83339,0-1.28256-.12091-1.85808-.68896l-8.94041-8.75747-.46337.12665c-.69137.1855-1.39226.27995-2.08262.27995-1.449,0-3.48235-.31392-4.94222-1.80946C-.07236,10.66245-.24291,7.90103.1872,6.71126c.16953-.46613.41788-.7053.73792-.71006.18822-.01699.52932-.05096.82387-.05096.19161,0,.26534.01563.28334.02038.57484.18618,2.06393,1.40313,2.86538,2.1261l.20826.1889.265-.0992c1.39294-.52252,2.43697-1.57029,2.93944-2.95032l.07678-.20724-1.56043-2.6065c-.3095-.32683-.57654-.93837-.51131-1.32024.08392-.48039.68288-.88197,1.0831-.98729.49636-.07678.94958-.11483,1.40653-.11483,2.14003,0,3.37669,1.03757,4.2145,1.9406,1.71468,1.84887,2.32825,4.53283,1.64197,7.17942l-.11264.43332,8.41938,8.47684c1.11096,1.10892,1.56788,3.16966.22454,4.75285-.67744.80179-1.62899,1.0736-2.69102,1.0736ZM9.66108,12.73895l9.80646,9.36736c.33702.33295.79398.51505,1.28762.51505.53102,0,1.04606-.20792,1.41333-.57145.35979-.35741.59319-.93905.59523-1.48195,0-.45254-.15322-.84528-.44302-1.13746l-9.16539-9.67734.10226-.26228c.88808-2.29326.76664-3.51823-.37727-5.5295-.61697-1.08649-1.64911-2.90615-4.95717-2.90615,0,0-.46612-.03478-.56167.2298-.11489.31815,1.77489,2.09131,1.77489,2.09131.38561.37983,1.02976,1.01447.86566,1.71841-.60644,2.56641-2.55316,4.39014-5.2079,4.87869l-.22355.04213s-2.14206-3.09935-3.18352-3.10445c-.20551-.00101-.20974.58937-.23692.78717-.19803,1.44117.93897,3.76934,2.16238,4.78992,1.17109.97574,2.4954,1.05796,3.86286,1.05999.67439,0,1.29849-.28538,1.90221-.56193l.58352-.24734Z" />
                            </svg>
                        </button>
                        <button class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--bg3)]" @click="openIncidentReport" v-if="!route.params.type">
                            <IncidentIcon size="16" />
                        </button>
                    </div>
                </div>
            </div>
            ...
            <FloatButton :hide-on="sortParent" title="インシデント報告" @action="openIncidentReport" class="fixed pc" v-if="!route.params.type">
                <template #icon>
                    <IncidentIcon size="18" />
                </template>
            </FloatButton>
        </div>
...
const openIncidentReport = () => {
    router.push({ name: 'emergency_contact', query: { type: 'incident' } })
}
watch(
    () => initialLoader.value,
    async (loading) => {
