<template>
    ...
    >
        <template #icon>
            <svg class="side-app-icon mr-1" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" style="width: auto; height: 15px;">
        ...
        <div class="mx-3 mb-3">
                    ...
                    >
                        <template #title="{ expanded }">
                            <PanelTitle :expanded="expanded">
                                <span class="side-notification !w-2 !min-w-2 !h-2 mr-2 custom-heartbeat" style="position: unset;"></span>
                                計画有給
                            </PanelTitle>
                        </template>
                        <template #body>
            ...
            <div v-if="data.data.pendingTimesheets.length" class="mx-3 mb-3">
                ...
                <div class="grid" :class="`grid-cols-${data.col.split('-')[2]}`">
                    <div class="p-2 cursor-pointer hover:bg-[var(--bg3)] rounded flex items-center text-[13px] overflow-hidden" v-for="item in data.data.pendingTimesheets">
                        <UserPanel disable-instant size="25" :user="item.user"/>
                        <div class="ml-2 flex w-full">
                            <div class="flex flex-col w-full">
                                <div class="flex justify-between w-full">
                                    <div class="">{{ item.user.name }}</div>
                                    <router-link :to="{name: 'timesheet', query: {user_id: item.user.id}}" class="jump-link ml-auto whitespace-nowrap" @click="">対応</router-link>
                                </div>
                                <div class="flex flex-col gap-1 mt-1">
                                    <template v-if="item.timecard && item.timecard.length">
                                        <div v-for="(timecard) in item.timecard" class="text-[12px]">
                                            日報申請 : {{ timecard.month }}月分<span class="text-[gray]">（{{timecard.count}}件）</span>
                                        </div>
                                    </template>
