        ...
        <div>
            ...
            <div ref="messageContainer" class="max-h-[50vh] overflow-auto bg-[var(--bg2)] px-[20px] py-[40px] m-[10px]">
                ...
                <TransitionGroup name="goal-comment" class="flex flex-col gap-[50px]" tag="div" v-if="item.comments && item.comments.length">
                    <ContactCommentItem
                        v-for="(report, mIndex) in item.comments"
                        :comment="report"
                    />
                </TransitionGroup>         
                <div class="p-[15px] text-[gray] justify-center" v-if="!item.comments.length">現在コメントはありません</div>                       
            </div>
...
import { MessageFile, UploadingFile, User } from '@/interface/globalInterface';
import { useFilePreview } from '@/store/filePreview';
import AiCorrection from '@/components/Global/AiCorrection.vue';
import AiIcon from '@/components/Icons/AiIcon.vue';
import ContactCommentItem from './ContactCommentItem.vue';
...
const commentRead = async () => {
  await api.post(`/contact/${props.item.id}/comment_read`)
}
...
const commentSendConfirm = async(num) => {
    ...
    const data = await api.post('/contact_create_comment', {
