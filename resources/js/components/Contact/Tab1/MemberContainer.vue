<template>
    <div>
        ...
        <div class="createBoardButton" style="position: fixed;" @click.stop="switchView">
    ...
    const sortableList = computed(() => {
        ...
        return memberList.value.map(position => {
            const filteredEmployees = position.employees.filter(employee => {
                ...
                return (
                    name.toLowerCase().includes(props.keyword.toLowerCase()) ||
                    email.toLowerCase().includes(props.keyword.toLowerCase()) ||
                    name_kana.toLowerCase().includes(props.keyword.toLowerCase()) ||
                    phone.includes(props.keyword)
                );
            });
            return {
                ...position,
