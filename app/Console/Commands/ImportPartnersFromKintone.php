...
class ImportPartnersFromKintone extends Command
{
    ...
    public function handle(KintoneClient $kintone): int
    {
        ...
        // Console runs without CommunityContext, so the BelongsToCommunity creating
        // hook cannot stamp community_id — assign the default (glowd) community here.
        $communityId = DB::table('communities')->where('slug', 'glowd')->value('id');
        $rows = array_map(fn ($r) => $r + ['community_id' => $communityId], $rows);
