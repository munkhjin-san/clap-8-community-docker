...
class ReportService
{
    ...
    private function monthly_performance_message()
    {
        ...
        $achieve = $this->weekday_get($month, '27');
        $contract_achieve = $this->weekday_get($month, '20');
        $message = <<<EOT
        [To:全員:]
        🔔お願い🔔
        翌月成果シート作成。
        ...
        各位
        お疲れ様です、経営管理本部です。
        {$nextMonth}月以降（2025年度）の成果目標作成をお願いいたします。

        期日：{$contract_achieve}まで
        期日までに最低２つあるように作成・申請してください。
        ※最終的にステータスが「目標を人事に申請中（人事対応中）」になっていれば申請できています。

        ①プロジェクトページをクリック
        ②自分が所属するメインのプロジェクト名をクリック
        ③“成果目標・昇給課題”の編集ボタンをクリック
        ④右下の“+”ボタンをクリックし、翌月以降に取り組む成果目標を設定
        ⑤“AI判定とフィードバック”をクリック。修正指示やアドバイス等の内容をもとに、必要に応じて修正を行う
        ⑥上長に申請した旨を報告し、承認を得る。
        EOT;
    ...
    private function monthly_shift_message()
    {
        ...
        [To:全員:]
        🔔お願い🔔
        翌月シフト、勤怠月締め、勤怠管理者変更、物品管理、3Sについて
        ...
        4.物品登録確認
        GLOWDへの物品登録状況が最新の状態か各自ご確認をお願いいたします。
        ※物品を移動させる場合は
        プロジェクトアプリ
        →所属プロジェクト
        →物品
        →該当物品の詳細から「移動」を選択し詳細を入力して下さい。

        5.3Sチェック
        {$month}月の3Sご回答をお願い致します。
        フルネーム・漢字・苗字と名前のスペースなしでの入力を宜しくお願いいたします。
        また、物品管理でのご自身の貸与品について変更がないか確認を必ずお願いいたします。
        全社員・パートナー社員必須（営業所に常駐してない場合も必須になります。）
        現在の職場でのルールを把握して、3Sを運用出来ているかの確認になります。
        [3Sフォーム]https://forms.gle/E6BkpHZauLWoeBm4A
        3Sとは「会社のお片付け」「お掃除活動」などの意味合いももちろんありますが、きれいにすることだけが目的ではありません。
        3S活動の目的は、「安全」で「効率的」で「快適」な職場を作ることです。
