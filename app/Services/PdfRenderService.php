...
class PdfRenderService
{
    ...
    public function render(FlowDefinition $definition, FlowRecord $record, array $template): Mpdf
    {
        ...
        foreach ($elements as $el) {
            if (($el['type'] ?? '') === 'table') { $table = $el; break; }
        }
        ...
        foreach ($elements as $el) {
            ...
            if ((float) ($el['y'] ?? 0) < $tableTop) $header[] = $el;
            else $after[] = $el;
        }
    ...
    {
        $size = (float) ($style['fontSize'] ?? 12);
        $pt = round($size * 0.75, 1); // px -> pt
        $align = $style['align'] ?? 'left';
        $color = $style['color'] ?? '#111';
        $weight = !empty($style['bold']) ? '700' : '400';
        $lh = $style['lineHeight'] ?? 1.4;
    ...
    private function renderTable(array $table, array $byKey): string
    {
        ...
        if (!$columns && $rows) {
            $keys = [];
            foreach ($rows as $row) {
                if (is_array($row)) foreach (array_keys($row) as $k) $keys[$k] = true;
            }
        ...
        if (!$amountKey) {
            foreach ($srcCols as $c) {
                if (in_array($c['input_type'] ?? '', ['number', 'formula', 'calc'], true)) { $amountKey = $c['key'] ?? null; break; }
            }
        }
        if (!$amountKey && $rows) {
            foreach ($columns as $c) {
                $k = $c['colKey'] ?? '';
                if ($k !== '' && collect($rows)->contains(fn ($r) => is_array($r) && is_numeric($r[$k] ?? null))) { $amountKey = $k; break; }
            }
        }

        $headStyle = $table['headStyle'] ?? [];
        $headBg = $headStyle['fill'] ?? '#f0f2f5';
        $borderColor = $table['borderColor'] ?? '#c9cfd8';
        $cellPt = round(((float) ($table['fontSize'] ?? 11)) * 0.75, 1);
        ...
        if (!empty($table['showSubtotal']) || !empty($table['showTax']) || !empty($table['showTotal'])) {
            ...
            $cur = $table['currency'] ?? '¥';
            $rows2 = [];
            if (!empty($table['showSubtotal'])) $rows2[] = ['小計', $subtotal];
            if (!empty($table['showTax'])) $rows2[] = ['消費税（' . rtrim(rtrim(number_format($rate * 100, 1), '0'), '.') . '%）', $taxAmt];
            if (!empty($table['showTotal'])) $rows2[] = ['合計', $total];
    ...
    {
        if (!$bound) return (string) ($el['fallback'] ?? '');
        ...
        if ($s === '' && isset($el['fallback'])) return (string) $el['fallback'];
    ...
    private function formatScalar($value, $fmt, $ctx = null): string
    {
        ...
        if (is_array($value)) {
            if (isset($value['label'])) return (string) $value['label']; // reference/lookup cells
            $flat = array_map(
    ...
    {
        $v = $bound['value'] ?? null;
        if (is_array($v) && isset($v[0]['url'])) return $v[0]['url'];
        return null;
    }

    /** Natural [width, height] in px of a data-URI or URL image; null when unmeasurable. */
