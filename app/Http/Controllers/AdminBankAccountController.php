...
class AdminBankAccountController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Confine to the active community's members (users are not community-scoped
        // by column, so an unscoped listing would span every community).
        $communityUserIds = app(\App\Services\Community\CommunityContext::class)->userIds();

        $rows = EmployeeBankAccount::query()
            ->with(['user:id,name,retire', 'updatedBy'])
            ->when($communityUserIds !== null, fn ($q) => $q->whereIn('user_id', $communityUserIds))
